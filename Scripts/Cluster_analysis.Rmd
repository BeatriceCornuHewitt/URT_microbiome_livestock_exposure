...
logistic_results_df_filtered <- logistic_results_df %>%
  filter(!grepl("^(age|gender|smoked|sampling|\\(Intercept\\))", rownames(logistic_results_df))) %>%
  select(Cluster, Variable, Estimate, Adjusted_p_value, `Std. Error`)

# Calculate Odds Ratios and Confidence Intervals
logistic_results_df_filtered <- logistic_results_df_filtered %>%
  mutate(
    OR = exp(Estimate),
...
logistic_results_df_filtered$Variable <- factor(logistic_results_df_filtered$Variable,
...
heatmap_data <- logistic_results_df_filtered %>%
  select(Variable, Cluster, OR) %>%
  dcast(Variable ~ Cluster, value.var = "OR")  # Reshape data for heatmap

# Convert to long format for ggplot
heatmap_long <- melt(heatmap_data, id.vars = "Variable", variable.name = "Cluster", value.name = "OR")


# Create the heatmap with italic formatting
heatmap_residential_expo <- ggplot(heatmap_long, aes(x = Cluster, y = Variable, fill = OR)) +
...
# Save the heatmap
ggsave("../Output/Cluster_analysis/CP_NP/Residential_livestock_expo_heatmap.svg", heatmap_residential_expo, width = 8, height = 6)
...
sample_data_df_cp_gf_np <- sample_data_cp_gf_np %>%
...
for (clust in unique(sample_data_df_cp_gf_np$cluster)) {
  ...
  model_multivariable <- glm(formula_multivariable, 
                              data = sample_data_df_cp_gf_np, 
                              family = binomial)
  
  # Extract summary statistics
  summary_stats <- as.data.frame(summary(model_multivariable)$coefficients)
  
  # Add cluster name for clarity
  summary_stats$Cluster <- clust
  
  # Add variable names explicitly
  summary_stats$Variable <- rownames(summary_stats)
  ...
  # Reorder columns to have 'Variable' and 'Cluster' at the end
  summary_stats <- summary_stats[, c("Variable", "Cluster", "Estimate", "Std. Error", "z value", "Pr(>|z|)", "Adjusted_p_value")]
  
  # Append to the results data frame
  logistic_results_df_cp_gf_np <- rbind(logistic_results_df_cp_gf_np, summary_stats)
}

# Optionally view the results
print(logistic_results_df_cp_gf_np)

# Ensure to have columns named properly in the final data frame
colnames(logistic_results_df_cp_gf_np) <- c("Variable", "Cluster", "Estimate", "Std. Error", "z value", "Pr(>|z|)", "Adjusted_p_value")

# Display the final results
print(logistic_results_df_cp_gf_np)


write.csv(logistic_results_df_cp_gf_np, "../Output/Cluster_analysis/CP_GF_NP/log_regression_results_cp_gf_np.csv")
...
#### Dot and whisker plot
```{r}
dot_whisker_plot_cp_gf_np <- ggplot(logistic_results_df_cp_gf_np_filtered, aes(x = Variable, y = OR, color = Cluster, group = Cluster)) +
  geom_point(size = 4, position = position_dodge(width = 1)) +  # Dodge to avoid overlap
  geom_errorbar(aes(ymin = Lower_CI, ymax = Upper_CI), width = 0.2, position = position_dodge(width = 1)) +  # Dodge for error bars
  scale_y_continuous(trans = 'log', breaks = scales::breaks_log()) +  # Log scale for OR
  labs(
    title = "Odds Ratios for Occupational Livestock Exposure",
    x = "Variables",
    y = "Odds Ratio (Log Scale)",
    color = "Cluster"
  ) +
...
for (clust in unique(sample_data_df_cp_gf_op$cluster)) {
  ...
  # Append to the results data frame
  logistic_results_df_cp_gf_op <- rbind(logistic_results_df_cp_gf_op, summary_stats)
}
...
# Ensure to have columns named properly in the final data frame
colnames(logistic_results_df_cp_gf_op) <- c("Variable", "Cluster", "Estimate", "Std. Error", "z value", "Pr(>|z|)", "Adjusted_p_value")

# Display the final results
print(logistic_results_df_cp_gf_op)


write.csv(logistic_results_df_cp_gf_op, "../Output/Cluster_analysis/CP_GF_OP/log_regression_results_cp_gf_op.csv")
...
logistic_results_df_cp_gf_op_filtered <- logistic_results_df_cp_gf_op %>%
  filter(!grepl("^(age|gender|smoked|sampling|\\(Intercept\\))", logistic_results_df_cp_gf_op$Variable)) %>%
  select(Cluster, Variable, Estimate, Adjusted_p_value, `Std. Error`)
...
# Reorder the variables for the plot
heatmap_data_cp_gf <- logistic_results_df_cp_gf_op_filtered %>%
  select(Variable, Cluster, OR) %>%
  dcast(Variable ~ Cluster, value.var = "OR")  # Reshape data for heatmap
...
# Save the heatmap
ggsave("../Output/Cluster_analysis/CP_GF_OP/Occupational_livestock_expo_heatmap.svg", heatmap_occupational_expo, width = 8, height = 6)


```
#### Dot and whisker plot
```{r}
dot_whisker_plot_cp_gf_op <- ggplot(logistic_results_df_cp_gf_op_filtered, aes(x = Variable, y = OR, color = Cluster, group = Cluster)) +
  geom_point(size = 4, position = position_dodge(width = 1)) +  # Dodge to avoid overlap
  geom_errorbar(aes(ymin = Lower_CI, ymax = Upper_CI), width = 0.2, position = position_dodge(width = 1)) +  # Dodge for error bars
  scale_y_continuous(trans = 'log', breaks = scales::breaks_log()) +  # Log scale for OR
  labs(
...
dot_whisker_plot_cp_gf_op

# Save the plot with increased height
ggsave("../Output/Cluster_analysis/CP_GF_OP/dot_whisker_plot_cp_gf_op.svg", dot_whisker_plot_cp_gf_op, width = 12, height = 8)
```
#### Visualisations for manuscript
```{r}
