...
metadata_KRD2022 <- left_join(metadata_df, KRD2022, by = "VGO3_ID")
rownames(metadata_KRD2022) <- rownames(metadata_df)
metadata_KRD2022 <- sample_data(metadata_KRD2022)

ps_complete_KRD <- ps_complete
ps_complete_KRD@sam_data <- metadata_KRD2022

saveRDS(ps_complete_KRD, "../Output/Phyloseq/3_ps_complete_KRD.rds")
```


# Microbial agent models - application
## BVB livestock data (from 2015)
...
Models were constructed on 2015 livestock data, so use this data for model application for our study population
...
ps_complete_KRD_CP <- subset_samples(ps_complete_KRD, population == "CP")
VGO3_IDs <- ps_complete_KRD_CP@sam_data$VGO3_ID
...
head(BVB_2015_filtered) 
str(BVB_2015_filtered) # 963 remaining
...
metadata_KRD <- data.frame(ps_complete_KRD@sam_data)
...
# Join with sam data 
metadata_KRD_RFmodels <- metadata_KRD %>%
  left_join(RF_predictions, by = "VGO3_ID")

# Make sure row names of new metadata are the same as the old metadata
rownames(metadata_KRD_RFmodels) <- rownames(metadata_KRD)

# Assign the updated dataframe back to the sam_data in the phyloseq object
ps_complete_KRD_RFmodels <- ps_complete_KRD
ps_complete_KRD_RFmodels@sam_data <- sample_data(metadata_KRD_RFmodels)

# Convert to character var to match with original_samdata_df
saveRDS(ps_complete_KRD_RFmodels, "../Output/Phyloseq/4_ps_complete_KRD_RFmodels.rds")
```

# Dispersion-modelled endotoxin data
Models developed for 2012 and 2020, we use the data from 2020 to estimate exposure to endotoxin
```{r}
# Read in data
...
# Check to see for how many participants we have missing data
ps_complete_KRD_RFmodels_CP <- subset_samples(ps_complete_KRD_RFmodels, population == "CP")
CP_IDs_ps <- unique(ps_complete_KRD_RFmodels_CP@sam_data$VGO3_ID)

# Find IDs in CP_IDs_ps that are not in endotoxin$respnr
missing_ids <- CP_IDs_ps[!(CP_IDs_ps %in% endotoxin$respnr)]
missing_ids # None missing in endotoxin dataset

# How many samples in the VGO3 data have misssing (NA) endotoxin values?
# Filter endotoxin for matching IDs
matching_rows <- endotoxin %>% 
  filter(respnr %in% CP_IDs_ps)
# Count how many have NA in the specified column
num_NA <- sum(is.na(matching_rows$EU_ANNUALAVG_PM10_2020))
num_NA # 239 VGO3 


# Add to phyloseq metadata
metadata_KRD_RFmodels <- ps_complete_KRD_RFmodels@sam_data
metadata_KRD_RFmodels_df <- data.frame(metadata_KRD_RFmodels)
...
metadata_endotoxin <- left_join(metadata_KRD_RFmodels_df, endotoxin, by = "VGO3_ID")
rownames(metadata_endotoxin) <- rownames(metadata_KRD_RFmodels)
metadata_endotoxin <- sample_data(metadata_endotoxin)

ps_complete_KRD_RFmodels_endotoxin <- ps_complete_KRD_RFmodels
ps_complete_KRD_RFmodels_endotoxin@sam_data <- metadata_endotoxin
```

# Standardising all livestock predictor variables
```{r}
# z-score standardisation to all livestock variables
ps_complete_KRD_RFmodels_endotoxin@sam_data$EU_ANNUALAVG_PM10_2020
ps_complete_KRD_RFmodels_endotoxin@sam_data$ecoli_RF_preds
ps_complete_KRD_RFmodels_endotoxin@sam_data$staph_RF_preds
ps_complete_KRD_RFmodels_endotoxin@sam_data$tetw_RF_preds
ps_complete_KRD_RFmodels_endotoxin@sam_data$meca_RF_preds
ps_complete_KRD_RFmodels_endotoxin@sam_data$KRD_nCowsWghtDist.3000m.sum
ps_complete_KRD_RFmodels_endotoxin@sam_data$KRD_nGoatsWghtDist.3000m.sum
ps_complete_KRD_RFmodels_endotoxin@sam_data$KRD_nPoultryWghtDist.3000m.sum
ps_complete_KRD_RFmodels_endotoxin@sam_data$KRD_nPigsWghtDist.3000m.sum
...
for (var in vars_to_standardise) {
  ...
  ps_complete_KRD_RFmodels_endotoxin@sam_data[[new_var]] <- scale(ps_complete_KRD_RFmodels_endotoxin@sam_data[[var]])
}

saveRDS(ps_complete_KRD_RFmodels_endotoxin, "../Output/Phyloseq/5_ps_complete_KRD_RFmodels_endotoxin.rds")
