...
required_packages_betadiv <- c(
  ...
  "colorspace", "scales", "ggpubr", "RColorBrewer", "paletteer", "rcartocolor", "microViz", "tidyr", "cowplot", "devtools","microViz", "remotes"
)
...
# Change Goat farm exposure categories to <2000m and >2000m due to limited numbers in the <1000m category for GP alone
ps@sam_data$GoatFarmExposureCategory[ps@sam_data$population == 'GP' & 
                                     ps@sam_data$GoatFarmExposureCategory %in% c("<1000m", "1000-2000m")] <- "<2000m"
...
# A total of 820 ASVs were found to be present at or above a level of confident detection (0.1% relative abundance) in at least 2 samples (n = 19710 ASVs excluded).
...
# A total of 659 ASVs were found to be present at or above a level of confident detection (0.1% relative abundance) in at least 2 samples (n = 19871 ASVs excluded).
...
m <- meta_to_df(ps) %>%
  mutate(across(all_of(t$`Variable name`), as.character)) %>%
...
bray_curtis_dist_NP <- vegdist(small_otu, method = "bray")
...
lst_CP <- c("gender", "age", "smoked_ever", "sampling_season", "flu_vacc", "pneumococcal_vacc", 
            ...
            "no_farm_visit_lastyr", "RVO2021_MindistGoatFarm50plus_NEG", "imm_def", "GoatFarmExposureCategory")
...
lst_GF <- c("gender", "age", "smoked_ever", "sampling_season", "flu_vacc", "pneumococcal_vacc", 
            ...
            "imm_def", "GoatFarmExposureCategory")
...
lst_GP <- c("gender", "age", "smoked_ever", "sampling_season", "flu_vacc", "pneumococcal_vacc", 
            ...
            "asthma_status", "COPD_status", "imm_def", "antibiotics_lastmth", "dyspnea", 
            "cough", "hypotension", "fever", "pain_breathing", "tachycardia", "tachypnea", 
            "confusion", "RVO2021_MindistGoatFarm50plus_NEG", "GoatFarmExposureCategory")
...
```{r}
univar_perm_all <- bind_rows(perm_NP, perm_OP)
write.xlsx(perm_ALL, "../Output/Beta_diversity/Univariable_PERMANOVAs/Univariable_permanova_results_NP_OP.xlsx")
```
## Visualise Univariable PERMANOVA results
```{r}
perm <- rbind(univar_perm_all, univar_perm_OP)
...
## PCoA plots
We also generated PCoA plots for the populations per niche

```{r PCoA_OP, ow = '90%', h = 5, w = 6}
small_ps@sam_data$population <- factor(small_ps@sam_data$population, levels = c("GF", "GP", "CP"))
p <- small_ps %>% 
  dist_calc("bray") %>%
...
<!-- ```{r} -->
<!-- # Prepare the data to conduct the analyses on the entire dataset -->
<!-- NP_otu_tab <- as.data.frame(otu_table(ps_NP_filt_RA)) %>% t() -->
<!-- NP_meta_df <- meta_to_df(ps_NP_filt_RA) -->

<!-- # Generate a Bray Curtis distance matrix -->
<!-- BC_dist_NP <- vegdist(NP_otu_tab, method = "bray") -->
<!-- BC_ord_NP <- BC_dist_NP %>% as.matrix() -->
...
<!-- CP_subset_df <- subset(NP_meta_df, population == "CP") -->
<!-- completeness_CP <- colSums(!is.na(CP_subset_df[, vars_all])) -->
<!-- print(completeness_CP) -->

<!-- GF_subset_df <- subset(NP_meta_df, population == "GF") -->
<!-- completeness_GF <- colSums(!is.na(GF_subset_df[, vars_all])) -->
<!-- print(completeness_GF) -->

<!-- GP_subset_df <- subset(NP_meta_df, population == "GP") -->
<!-- completeness_GP <- colSums(!is.na(GP_subset_df[, vars_all])) -->
<!-- print(completeness_GP) -->

<!-- # Use beta_df to prepare data -->
<!-- NP_complete_ps <- NA_remove(BC_ord_NP, meta_to_df(ps_NP_filt_RA), vars_all) -->
<!-- # Print the number of rows removed -->
<!-- print(NP_complete_ps$rows_removed) -->

<!-- # Run adonis2 with the specified variables -->
<!-- permanova_result_NP_all <- adonis2(NP_complete_ps$ord_sub ~ ., permutations = 999, data = NP_complete_ps$m_sub[, vars_all]) -->
<!-- print(permanova_result_NP_all) -->

<!-- # Perform pairwise PERMANOVA tests -->
<!-- pairwise_perm_anova <- pairwise.adonis2(NP_complete_ps$ord_sub ~ ., data = NP_complete_ps$m_sub[, vars_all], permutations = 999, method = "bray") -->
<!-- print(pairwise_perm_anova) -->
<!-- ``` -->
...
# Run adonis2 with the specified variables
permanova_result_NP_exp_CP_GF <- adonis2(NP_CP_GF_complete_ps$ord_sub ~ ., permutations = 999, data = NP_CP_GF_complete_ps$m_sub[, vars_exp_CP_GF])
print(permanova_result_NP_exp_CP_GF)

# Perform pairwise PERMANOVA tests
pairwise_perm_anova_NP_exp_CP_GF <- pairwise.adonis2(NP_CP_GF_complete_ps$ord_sub ~ ., data = NP_CP_GF_complete_ps$m_sub[, vars_exp_CP_GF], permutations = 999, method = "bray")
print(pairwise_perm_anova_NP_exp_CP_GF)

# Perform pairwise PERMANOVA tests
pairwise_perm_anova_NP_exp_CP_GF <- pairwise.adonis2(NP_CP_GF_complete_ps$ord_sub ~ ., 
                                                    data = NP_CP_GF_complete_ps$m_sub[, vars_exp_CP_GF], 
                                                    permutations = 999, method = "bray")

# Filter rows containing "GoatFarmExposure" and select R2 and p-value columns
results_filtered <- subset(pairwise_perm_anova_NP_exp_CP_GF$`1000-2000m_vs_<1000m`, grepl("GoatFarmExposureCategory", rownames(pairwise_perm_anova_NP_exp_CP_GF)))
results_filtered <- results_filtered[, c("R2", "p")]

# Save filtered results to CSV
write.csv(results_filtered, file = "../Output/Beta_diversity/Multivariable_PERMANOVAs/Pairwise_perm_CPGF.csv", row.names = TRUE)
```
...
# Run adonis2 with the specified variables
permanova_result_NP_exp_GP_GF <- adonis2(NP_GP_GF_complete_ps$ord_sub ~ ., permutations = 999, data = NP_GP_GF_complete_ps$m_sub[, vars_exp_GP_GF])
print(permanova_result_NP_exp_GP_GF)
...
<!-- ```{r} -->
<!-- # Prepare the data to conduct the analyses on the entire dataset -->
<!-- OP_otu_tab <- as.data.frame(otu_table(ps_OP_filt_RA)) %>% t() -->
<!-- OP_meta_df <- meta_to_df(ps_OP_filt_RA) -->

<!-- # Generate a Bray Curtis distance matrix -->
<!-- BC_dist <- vegdist(OP_otu_tab, method = "bray") -->
<!-- BC_ord <- BC_dist %>% as.matrix() -->

<!-- # Variables for multivariable PERMANOVA for all populations together  -->
<!-- vars_all <- c("population", "gender", "age", "smoked_ever", "sampling_season", "GoatFarmExposureCategory") -->

<!-- # Check completeness of variables for all populations separately -->
<!-- CP_subset_df <- subset(OP_meta_df, population == "CP") -->
<!-- completeness_CP <- colSums(!is.na(CP_subset_df[, vars_all])) -->
<!-- print(completeness_CP) -->

<!-- GF_subset_df <- subset(OP_meta_df, population == "GF") -->
<!-- completeness_GF <- colSums(!is.na(GF_subset_df[, vars_all])) -->
<!-- print(completeness_GF) -->

<!-- GP_subset_df <- subset(OP_meta_df, population == "GP") -->
<!-- completeness_GP <- colSums(!is.na(GP_subset_df[, vars_all])) -->
<!-- print(completeness_GP) -->

<!-- # Use beta_df to prepare data -->
<!-- OP_complete_ps <- NA_remove(BC_ord, meta_to_df(ps_OP_filt_RA), vars_all) -->
<!-- # Print the number of rows removed -->
<!-- print(OP_complete_ps$rows_removed) -->

<!-- # Run adonis2 with the specified variables -->
<!-- permanova_result_OP_all <- adonis2(OP_complete_ps$ord_sub ~ ., permutations = 999, data = OP_complete_ps$m_sub[, vars_all]) -->
<!-- print(permanova_result_OP_all) -->

<!-- # Perform pairwise PERMANOVA tests -->
<!-- pairwise_permanova_OP_all <- pairwise.adonis2(OP_complete_ps$ord_sub ~ ., data = OP_complete_ps$m_sub[, vars_all], permutations = 999, method = "bray") -->
<!-- print(pairwise_permanova_OP_all) -->
<!-- ``` -->
### Population (miseq runs: 54,56,57,58) 
```{r}
...
# Run adonis2 with the specified variables
permanova_result_OP_exp_CP_GF <- adonis2(OP_CP_GF_complete_ps$ord_sub ~ ., permutations = 999, data = OP_CP_GF_complete_ps$m_sub[, vars_exp_CP_GF])
print(permanova_result_OP_exp_CP_GF)
...
# Run adonis2 with the specified variables
permanova_result_OP_exp_GP_GF <- adonis2(OP_GP_GF_complete_ps$ord_sub ~ ., permutations = 999, data = OP_GP_GF_complete_ps$m_sub[, vars_exp_GP_GF])
print(permanova_result_OP_exp_GP_GF)
...
```
# Visualise PERMANOVA results in heatmaps
## Population (miseq runs: 54,56,57,58) 
```{r}
...
variable_order <- c("Smoking status" ,"Season of sampling","Age (years)","Gender", "Population")
...
```
# Examine the impact of chicken farms
# Goat farm exposure on CP NOT including chicken farm exposure (2000m)
```{r}
# Subset the NP ps object to contain only CP samples
ps_NP_CP_filt_RA <- subset_samples(ps_NP_filt_RA, !(population %in% c("GP", "GF")))
...
# Check completeness of variables for CP & GF
completeness_CP <- colSums(!is.na(NP_CP_meta_df[, vars_exp_CP_2]))
print(completeness_CP)
...
# Run adonis2 with the specified variables
permanova_result_NP_exp_CP <- adonis2(NP_CP_complete_ps$ord_sub ~ ., permutations = 999, data = NP_CP_complete_ps$m_sub[, vars_exp_CP])
print(permanova_result_NP_exp_CP)

write.csv(permanova_result_NP_exp_CP, "../Output/Beta_diversity/Multivariable_PERMANOVAs/Chicken farm test/multivar_permanova_CP_nochickenexpo.csv")


# Perform pairwise PERMANOVA tests
pairwise_perm_anova_NP_exp_CP <- pairwise.adonis2(NP_CP_complete_ps$ord_sub ~ ., data = NP_CP_complete_ps$m_sub[, vars_exp_CP], permutations = 999, method = "bray")
print(pairwise_perm_anova_NP_exp_CP)

```
# Goat farm exposure on CP only including chicken farm exposure (2000m)
```{r}
# Subset the NP ps object to contain only CP samples
ps_NP_CP_filt_RA <- subset_samples(ps_NP_filt_RA, !(population %in% c("GP", "GF")))
...
# Variables for multivariable PERMANOVA for all populations together 
vars_exp_CP_2 <- c("GoatFarmExposureCategory", "gender", "age", "smoked_ever", "sampling_season", "ChickenFarm2000m")

# Check completeness of variables for CP & GF
completeness_CP_2 <- colSums(!is.na(NP_CP_meta_df[, vars_exp_CP_2]))
print(completeness_CP_2)

# Use beta_df to prepare data
NP_CP_complete_ps_2 <- NA_remove(BC_ord_NP_CP, meta_to_df(ps_NP_CP_filt_RA), vars_exp_CP_2)
# Print the number of rows removed
print(NP_CP_complete_ps_2$rows_removed)

# Run adonis2 with the specified variables
permanova_result_NP_exp_CP_2 <- adonis2(NP_CP_complete_ps_2$ord_sub ~ ., permutations = 999, data = NP_CP_complete_ps_2$m_sub[, vars_exp_CP_2])
print(permanova_result_NP_exp_CP_2)

write.csv(permanova_result_NP_exp_CP_2, "../Output/Beta_diversity/Multivariable_PERMANOVAs/Chicken farm test/multivar_permanova_CP_chickenexpo.csv")


# Perform pairwise PERMANOVA tests
pairwise_perm_anova_NP_exp_CP_2 <- pairwise.adonis2(NP_CP_complete_ps_2$ord_sub ~ ., data = NP_CP_complete_ps_2$m_sub[, vars_exp_CP_2], permutations = 999, method = "bray")
print(pairwise_perm_anova_NP_exp_CP_2)
```
# PCoAs
## NP vs OP
```{r}
...
ggsave("../Output/Beta_diversity/PCoAs/PCoA_NP_OP.svg", PCoA_population_NP_OP, device = "svg", width  = 8, height = 6)
```
## Population 
### NP
```{r}
...
PCoA_population_NP_CPGF_1and2 <- plot_ordination(
...
PCoA_population_NP_CPGF_1and2

ggsave("../Output/Beta_diversity/PCoAs/PCoA_NP_CPGF.svg",PCoA_population_NP_CPGF_1and2,device = "svg", width  = 8, height = 6)
```
### OP 
```{r}
# Goat farm exposure on the control population 
...
PCoA_population_OP_CPGF_1and2 <- plot_ordination(
  ...
  annotate("text", x = 0.15, y = -0.53, label = bquote(italic(R)^2 == .(round(permanova_result_OP_exp_CP_GF$R2[rownames(permanova_result_OP_exp_CP_GF) == "GoatFarmExposureCategory"], 3))), size = 6, hjust = 0, color = "black") +
  annotate("text", x = 0.15, y = -0.6, label = bquote(italic(p)~"value" == .(round(permanova_result_OP_exp_CP_GF$`Pr(>F)`[rownames(permanova_result_OP_exp_CP_GF) == "GoatFarmExposureCategory"], 3))), size = 6, hjust = 0, color = "black") +
  stat_ellipse(aes(colour = GoatFarmExposureCategory, fill = GoatFarmExposureCategory), size = 1, show.legend = FALSE) +
  guides(color = guide_legend(override.aes = list(size = 8))) # Adjust the size of the legend key

PCoA_population_OP_CPGF_1and2

ggsave("../Output/Beta_diversity/PCoAs/PCoA_OP_CPGF.svg",PCoA_population_OP_CPGF_1and2,device = "svg", width  = 8, height = 6)
```
...
PCoA_NP_GPGF_1and2 <- plot_ordination(
  ...
  annotate("text", x = 0.30, y = -0.51, label = bquote(italic(R)^2 == .(round(permanova_result_NP_exp_GP_GF$R2[rownames(permanova_result_NP_exp_GP_GF) == "GoatFarmExposureCategory"], 3))), size = 6, hjust = 0, color = "black") +
  annotate("text", x = 0.30, y = -0.6, label = bquote(italic(p)~"value" == .(round(permanova_result_NP_exp_GP_GF$`Pr(>F)`[rownames(permanova_result_NP_exp_GP_GF) == "GoatFarmExposureCategory"], 3))), size = 6, hjust = 0, color = "black") +
  stat_ellipse(aes(colour = GoatFarmExposureCategory, fill = GoatFarmExposureCategory), size = 1, show.legend = FALSE) +
  guides(color = guide_legend(override.aes = list(size = 8))) 


PCoA_NP_GPGF_1and2
ggsave("../Output/Beta_diversity/PCoAs/PCoA_NP_GPGF.svg", PCoA_NP_GPGF_1and2,device = "svg", width  = 8, height = 6)
```
...
# Generate the plot
PCoA_population_OP_GPGF_1and2 <- plot_ordination(
...
PCoA_population_OP_GPGF_1and2
ggsave("../Output/Beta_diversity/PCoAs/PCoA_OP_GPGF.svg", PCoA_population_OP_GPGF_1and2,device = "svg", width  = 8, height = 6)
```
