---
title: "Descriptive statistics - VGO3"
---
# Packages
```{r}
library(dplyr)
...
meta[variables_to_factor] <- lapply(meta[variables_to_factor], factor)

sum(meta$population=="CP") # n = 969
sum(meta$population=="GP") # n = 108
sum(meta$population=="GF") # n = 100

write.csv(meta,"../Output/Descriptives/VGO3_population_data.csv" )
```
...
write.csv(meta_uniq_16S_noblanks,"../Output/Descriptives/VGO3_population_data_included_16S.csv" ) # 1172 samples
```
...
meta_uniq_noblanks$gender <- 
  factor(meta_uniq_noblanks$gender, levels=c(0,1),
         labels=c("Male", 
                  "Female"))
meta_uniq_noblanks$smoked_ever <- 
  factor(meta_uniq_noblanks$smoked_ever, levels=c(0,1,2),
         labels=c("No", 
                  "Ex-smoker",
                  "Smoker"))
meta_uniq_noblanks$flu_vacc <- 
  factor(meta_uniq_noblanks$flu_vacc, levels=c(0,1),
         labels=c("No", 
                  "Yes"))
meta_uniq_noblanks$pneumococcal_vacc <- 
  factor(meta_uniq_noblanks$pneumococcal_vacc, levels=c(0,1),
         labels=c("No", 
                  "Yes"))
meta_uniq_noblanks$animal_contact_lastmth<- 
  factor(meta_uniq_noblanks$animal_contact_lastmth, levels=c(0,1),
         labels=c("No", 
                  "Yes"))
meta_uniq_noblanks$abroad_lastmth<- 
  factor(meta_uniq_noblanks$abroad_lastmth, levels=c(0,1),
         labels=c("No", 
                  "Yes"))
meta_uniq_noblanks$imm_def<- 
  factor(meta_uniq_noblanks$imm_def, levels=c(0,1),
         labels=c("No", 
                  "Yes"))


label(meta_uniq_noblanks$gender) <- "Gender"
label(meta_uniq_noblanks$age) <- "Age"
label(meta_uniq_noblanks$smoked_ever) <- "Smoking status"
label(meta_uniq_noblanks$sampling_season) <- "Sampling season"
label(meta_uniq_noblanks$flu_vacc) <- "Influenza vaccination"
label(meta_uniq_noblanks$pneumococcal_vacc) <- "Pneumococcal vaccination"
label(meta_uniq_noblanks$animal_contact_lastmth) <- "Animal contact in the last month"
label(meta_uniq_noblanks$abroad_lastmth) <- "Abroad in the last month"
label(meta_uniq_noblanks$imm_def) <- "Immunocompromised"
...
result_table <- table1(
  ~ gender + age  + smoked_ever + sampling_season + imm_def + flu_vacc + pneumococcal_vacc + animal_contact_lastmth + abroad_lastmth | population,
  data = meta_uniq_noblanks,
   overall=F, 
  extra.col=list(`P-value`=pvalue))



write.csv(result_table, file = "../Output/Descriptives/table1.csv")
```
# Goat farm exposure - CP and GP 
```{r}
# Select CPs and GPs from the dataset
# Exclude rows where population is "Goat farmers"
meta_uniq_noblanks_CP_GP <- meta_uniq_noblanks[meta_uniq_noblanks$population != "Goat farmers", ]
meta_uniq_noblanks_CP_GP$population <- droplevels(meta_uniq_noblanks_CP_GP$population)


# Create a table of counts
counts_table <- table(meta_uniq_noblanks_CP_GP$population, meta_uniq_noblanks_CP_GP$GoatFarmExposureCategory)
...
for (variable in variables) {
  frequency_table <- table(meta_uniq_noblanks_CP_GP[[variable]])
...
goat_farms_1000m <- meta_uniq_noblanks_CP_GP$RVO2021_nGoatFarm_1000m
chicken_farms_1000m <- meta_uniq_noblanks_CP_GP$RVO2021_nChickenFarm_1000m
goat_farms_3000m <- meta_uniq_noblanks_CP_GP$RVO2021_nGoatFarm_3000m
chicken_farms_3000m <- meta_uniq_noblanks_CP_GP$RVO2021_nChickenFarm_3000m
# Check distribution
...
participants_with_chicken_farms_1000m <- sum(!is.na(meta_uniq_noblanks_CP_GP$RVO2021_nChickenFarm_1000m) & meta_uniq_noblanks_CP_GP$RVO2021_nChickenFarm_1000m != 0)
print(paste("Number of participants with a chicken farm within 1000m:", participants_with_chicken_farms_1000m))

# Number of participants with a chicken farm within 1000m (ignoring NA values)
participants_with_goat_farms_1000m <- sum(!is.na(meta_uniq_noblanks_CP_GP$RVO2021_nGoatFarm_1000m) & meta_uniq_noblanks_CP_GP$RVO2021_nGoatFarm_1000m != 0)
print(paste("Number of participants with a goat farm within 1000m:", participants_with_goat_farms_1000m))
```
# Descriptive - CP
```{r}
meta_uniq_noblanks_CP <- meta_uniq_noblanks %>%
  filter(population == "Control population")

meta_uniq_noblanks$ed
# Use table1 to create table 
meta_uniq_noblanks_CP$gender <- 
  factor(meta_uniq_noblanks_CP$gender, levels=c(0,1),
         labels=c("Male", 
                  "Female"))

meta_uniq_noblanks_CP$pneumonia_lastyr <- 
  factor(meta_uniq_noblanks_CP$pneumonia_lastyr, levels=c(0,1),
         labels=c("No", 
                  "Yes"))
 
meta_uniq_noblanks_CP$pneumonia_last3yr <- 
  factor(meta_uniq_noblanks_CP$pneumonia_last3yr, levels=c(0,1,2,3),
         labels=c("No", 
                  "Once",
                  "Twice",
                  "Three times"))

meta_uniq_noblanks_CP$acute_resp_infec_lastyr <- 
  factor(meta_uniq_noblanks_CP$acute_resp_infec_lastyr, levels=c(0,1),
         labels=c("No", 
                  "Yes"))

meta_uniq_noblanks_CP$covid19 <- 
  factor(meta_uniq_noblanks_CP$covid19, levels=c(0,1),
         labels=c("No", 
                  "Yes"))	


meta_uniq_noblanks_CP$antibiotics_lastyr<- 
  factor(meta_uniq_noblanks_CP$antibiotics_lastyr, levels=c(0,1),
         labels=c("No", 
                  "Yes"))	

meta_uniq_noblanks_CP$antacids_lastyr<- 
  factor(meta_uniq_noblanks_CP$antacids_lastyr, levels=c(0,1),
         labels=c("No", 
                  "Yes"))	

meta_uniq_noblanks_CP$lung_med <- 
  factor(meta_uniq_noblanks_CP$lung_med, levels=c(0,1),
         labels=c("No", 
                  "Yes"))	

meta_uniq_noblanks_CP$beta_blockers <- 
  factor(meta_uniq_noblanks_CP$beta_blockers, levels=c(0,1),
         labels=c("No", 
                  "Yes"))	

meta_uniq_noblanks_CP$heart_vascu_disorder	 <- 
  factor(meta_uniq_noblanks_CP$heart_vascu_disorder	, levels=c(0,1),
         labels=c("No", 
                  "Yes"))	

meta_uniq_noblanks_CP$diabetes	 <- 
  factor(meta_uniq_noblanks_CP$diabetes	, levels=c(0,1),
         labels=c("No", 
                  "Yes"))	

meta_uniq_noblanks_CP$nasal_allergy	 <- 
  factor(meta_uniq_noblanks_CP$nasal_allergy	, levels=c(0,1),
         labels=c("No", 
                  "Yes"))	

meta_uniq_noblanks_CP$asthma_ever	 <- 
  factor(meta_uniq_noblanks_CP$asthma_ever	, levels=c(0,1),
         labels=c("No", 
                  "Yes"))	

meta_uniq_noblanks_CP$COPD_emphysema_diag	 <- 
  factor(meta_uniq_noblanks_CP$COPD_emphysema_diag	, levels=c(0,1),
         labels=c("No", 
                  "Yes"))	

meta_uniq_noblanks_CP$EAA_diag	 <- 
  factor(meta_uniq_noblanks_CP$EAA_diag	, levels=c(0,1),
         labels=c("No", 
                  "Yes"))

meta_uniq_noblanks_CP$cort_inhaler_lastyr	 <- 
  factor(meta_uniq_noblanks_CP$cort_inhaler_lastyr	, levels=c(0,1),
         labels=c("No", 
                  "Yes"))

meta_uniq_noblanks_CP$smoked_ever <- 
  factor(meta_uniq_noblanks_CP$smoked_ever, levels=c(0,1,2),
         labels=c("No", 
                  "Ex-smoker",
                  "Smoker"))


label(meta_uniq_noblanks_CP$gender) <- "Gender"
label(meta_uniq_noblanks_CP$age) <- "Age"
label(meta_uniq_noblanks_CP$pneumonia_lastyr) <- "Pneumonia in the last year"
label(meta_uniq_noblanks_CP$pneumonia_last3yr) <- "Pneumonia in the last 3 Years"
label(meta_uniq_noblanks_CP$acute_resp_infec_lastyr) <- "Acute resp infection in the last year"
label(meta_uniq_noblanks_CP$covid19) <- "COVID-19"
label(meta_uniq_noblanks_CP$antibiotics_lastyr) <- "Antibiotics in the last year"
label(meta_uniq_noblanks_CP$antacids_lastyr) <- "Antacids in the last year"
label(meta_uniq_noblanks_CP$lung_med) <- "Taking lung medication"
label(meta_uniq_noblanks_CP$beta_blockers) <- "Taking beta blockers"
label(meta_uniq_noblanks_CP$heart_vascu_disorder) <- "Heart/vascular disorder"
label(meta_uniq_noblanks_CP$diabetes) <- "Diabetes"
label(meta_uniq_noblanks_CP$nasal_allergy) <- "Nasal Allergy"
label(meta_uniq_noblanks_CP$asthma_ever) <- "Asthma"
label(meta_uniq_noblanks_CP$COPD_emphysema_diag) <- "COPD/Emphysema Diagnosis"
label(meta_uniq_noblanks_CP$EAA_diag) <- "EAA diagnosis"
label(meta_uniq_noblanks_CP$cort_inhaler_lastyr) <- "corticosteroid inhaler in the last year"
label(meta_uniq_noblanks_CP$smoked_ever) <- "Smoking status"
label(meta_uniq_noblanks_CP$cigarettes_per_day) <- "Cigarettes smoked per day"
label(meta_uniq_noblanks_CP$RVO2021_nGoatFarm_3000m) <- "Number of goat farms within 3000m"
label(meta_uniq_noblanks_CP$RVO2021_nGoats_3000m) <- "Number of goats within 3000m"
label(meta_uniq_noblanks_CP$RVO2021_MindistGoatFarm50plus) <- "Minimum distance to nearest goat farm (50+)"
...
table1_CP <- table1(
  ...
  data = meta_uniq_noblanks_CP)

write.csv(table1_CP, file = "../Output/Descriptives/table1_CP.csv")


hist(meta_uniq_noblanks_CP$RVO2021_nGoats_3000m)
```
# Table for Aniek
# All participants 
```{r}
# Use table1 to create table 
meta$gender <- 
  factor(meta$gender, levels=c(0,1),
         labels=c("Male", 
                  "Female"))
meta$smoked_ever <- 
  factor(meta$smoked_ever, levels=c(0,1,2),
         labels=c("No", 
                  "Ex-smoker",
                  "Smoker"))
meta$asthma_ever <- 
  factor(meta$asthma_ever, levels=c(0,1),
         labels=c("No", 
                  "Yes"))
meta$COPD_emphysema_diag <- # N.B for CP and GF this diagnosis is COPD & emphysema but for GPs it is just COPD
  factor(meta$COPD_emphysema_diag, levels=c(0,1),
         labels=c("No", 
                  "Yes"))
meta$lung_med <- 
  factor(meta$lung_med, levels=c(0,1),
         labels=c("No", 
                  "Yes"))
meta$imm_def <- 
  factor(meta$imm_def, levels=c(0,1),
         labels=c("No", 
                  "Yes"))
meta$flu_vacc <- 
  factor(meta$flu_vacc, levels=c(0,1),
         labels=c("No", 
                  "Yes"))
meta$pneumococcal_vacc <- 
  factor(meta$pneumococcal_vacc, levels=c(0,1),
         labels=c("No", 
                  "Yes"))
meta$antibiotics_lastyr <- # for CP and GF
  factor(meta$antibiotics_lastyr, levels=c(0,1),
         labels=c("No", 
                  "Yes"))
meta$antibiotics_lastmth <- # for GP
  factor(meta$antibiotics_lastmth, levels=c(0,1),
         labels=c("No", 
                  "Yes"))
meta$abroad_lastmth<- 
  factor(meta$abroad_lastmth, levels=c(0,1),
         labels=c("No", 
                  "Yes"))
meta$GoatFarmwithin2000m <- 
  factor(meta$GoatFarmwithin2000m, levels=c(0,1),
         labels=c("No", 
                  "Yes"))
meta$GoatFarmwithin1000m <- 
  factor(meta$GoatFarmwithin1000m, levels=c(0,1),
         labels=c("No", 
                  "Yes"))
meta$GoatFarmwithin500m <- 
  factor(meta$GoatFarmwithin500m, levels=c(0,1),
         labels=c("No", 
                  "Yes"))
meta$animal_contact_lastmth<- 
  factor(meta$animal_contact_lastmth, levels=c(0,1),
         labels=c("No", 
                  "Yes"))

label(meta$sampling_season) <- "Sampling season"
label(meta$age) <- "Age"
label(meta$gender) <- "Gender"
label(meta$smoked_ever) <- "Smoking status"
label(meta$asthma_ever) <- "Asthma (ever)"
label(meta$COPD_emphysema_diag) <- "COPD/emphysema"
label(meta$lung_med) <- "Lung medication used"
label(meta$imm_def) <- "Immunocompromised"
label(meta$flu_vacc) <- "Influenza vaccination"
label(meta$pneumococcal_vacc) <- "Pneumococcal vaccination"
label(meta$antibiotics_lastmth) <- "Antibiotic use in the last month (GP)"
label(meta$antibiotics_lastyr) <- "Antibiotic use in the last year (CP and GF)"
label(meta$abroad_lastmth) <- "Abroad in the last month"
label(meta$GoatFarmwithin2000m) <- "Goat farm within 2000m (CP and GP)"
label(meta$GoatFarmwithin1000m) <- "Goat farm within 1000m (CP and GP)"
label(meta$GoatFarmwithin500m) <- "Goat farm within 500m (CP and GP)"
label(meta$animal_contact_lastmth) <- "Animal contact in the last month"



All_populations <- table1(
  ~ sampling_season + age + gender + smoked_ever + asthma_ever + COPD_emphysema_diag + lung_med + imm_def + flu_vacc + pneumococcal_vacc +antibiotics_lastmth + antibiotics_lastyr + abroad_lastmth + GoatFarmwithin2000m + GoatFarmwithin1000m + GoatFarmwithin500m + animal_contact_lastmth| population,
  data = meta,
  overall=F, 
  extra.col=list(`P-value`=pvalue))


write.csv(All_populations, file = "../Output/Descriptives/All_populations_table1.csv")
```
# Only participants with at least 1 OP or NP sample sequenced
```{r}
# Use table1 to create table 
...
All_populations <- table1(
  ~ sampling_season + age + gender + smoked_ever + asthma_ever + COPD_emphysema_diag + lung_med + imm_def + flu_vacc + pneumococcal_vacc + animal_contact_lastmth + abroad_lastmth + GoatFarmwithin2000m + GoatFarmwithin500m | population,
