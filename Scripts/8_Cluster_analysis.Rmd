...
# Packages
```{r}
library(tidyverse);library(magrittr);library(glue);library(here);library(ggtext);library(fs);library(phyloseq);library(microbiomer);library(vegan);library(microbiome);library(decontam);library(reshape2);library(colorspace);library(scales);library(ggpubr);library(ggvenn);library(ggnewscale);library(RColorBrewer);library(paletteer);library(broom);library(rcartocolor);library(dendextend);library(ggmosaic);library(patchwork); library(fpc); library(ggdendro); library(nnet); library(gridExtra)
...
ps_RA_cpall_gf_np <- ps_cpall_gf %>% pres_abund_filter() %>% to_RA() %>%
  prune_samples(sample_data(ps_cpall_gf)$niche == "NP", .)  %>% 
    # Ensure only unique VGO3_ID samples are kept
  prune_samples(
    !duplicated(sample_data(.)$VGO3_ID), 
    .
  )
...
cluster_assignments_8 <- cutree(dend_cpall_gf_np, k = 8)
...
cluster_assignments_9 <- cutree(dend_cpall_gf_np, k = 9)
...
cluster_assignments_10 <- cutree(dend_cpall_gf_np, k = 10)
...
cluster_assignments_11 <- cutree(dend_cpall_gf_np, k = 11)
...
cluster_assignments_12 <- cutree(dend_cpall_gf_np, k = 12)
...
cluster_assignments_13 <- cutree(dend_cpall_gf_np, k = 13)
...
cluster_assignments_14 <- cutree(dend_cpall_gf_np, k = 14)
...
cluster_assignments_15 <- cutree(dend_cpall_gf_np, k = 15)
...
dend_9clusters <- color_branches(dend_cpall_gf_np, k = 9, groupLabels=T)
plot(stats::dendrapply(dend_9clusters, noLabel))
cluster_assignments_9 <- cutree(dend_9clusters, k = 9)
cluster_counts_9 <- table(cluster_assignments_9)
print(cluster_counts_9)
...
plot(stats::dendrapply(dend_8clusters, noLabel))
cluster_assignments_8 <- cutree(dend_cpall_gf_op, k = 8)
cluster_counts_8 <- table(cluster_assignments_8)
print(cluster_counts_8)
...
plot(stats::dendrapply(dend_12clusters, noLabel))
# check number per cluster
cluster_assignments_12 <- cutree(dend_cpall_gf_op, k = 12)
cluster_counts_12 <- table(cluster_assignments_12)
print(cluster_counts_12)
...
# The function 'cuttree' is used to cut the tree at k = 7 as determined by the indices (see peak in CH index)
groups_cpall_gf_op_c7 <- cutree(hc_cpall_gf_op_clust, k=7)
ps_RA_cpall_gf_op@sam_data$cluster <- groups_cpall_gf_op_c7[order(rownames(ps_RA_cpall_gf_op@sam_data))]
...
top_3_asvs_df_cpall_gf_op$Cluster_Name

# Shorten the names made above for plotting 
# Define the new cluster names in the same order as the original clusters
new_cluster_names_cpall_gf_op <- c(
  "Strep(4)/Veill(6)/Prev_hist(12) clust",
  "Neis(13)/Strep(8)/Hae(14) clust",
  "Strep(8)/Gem(27) clust",
  "Veill(6)/Strep(4)/Prev(16) cluster",
  "Fuso_nuc(72)/Por_endo(76)/Prev(128) clust",
  "Strep(4) clust",
  "Strep(10)/Roth_muci(49)/Veill(6) clust"
)
...
bar_clusters_cpall_gf_op <- ggplot(data = cluster_counts_cpall_gf_op, aes(x = Cluster, y = (Proportion*100), fill = Cluster)) +
  ...
  labs(title = "Bar Plot of Clusters in Control Population and Goat farmers",
       ...
       y = "% of samples",          # Y-axis represents proportions
       fill = "Cluster") +
  scale_fill_manual(values = c(paletteer_d("pals::stepped")[c(16,9,4,3,20,18,5)])) +
  theme_minimal() +
  theme(axis.text.x = element_blank(),      # Remove x-axis labels if needed
        axis.ticks.x = element_blank(),     # Remove x-axis ticks if needed
        legend.position = "right")
...
otu_hm_cpall_gf_op_order <- otu_hm_cpall_gf_op[order(rowMeans(otu_hm_cpall_gf_op), decreasing = T)[1:40], hc_cpall_gf_op_clust$order]
...
meta_hm_cpall_gf_op_order$cluster
...
# Ensure OTU data is ordered by hierarchical clustering order
otu_hm_cpall_gf_op_order <- otu_hm_cpall_gf_op[order(rowMeans(otu_hm_cpall_gf_op), decreasing = TRUE), hc_cpall_gf_op_clust$order]
meta_hm_cpall_gf_op_order <- meta_hm_cpall_gf_op_order[hc_cpall_gf_op_clust$order, ]

# Overall heatmap
hm_cpall_gf_op <- hm_cpall_gf_op_data %>%
  ggplot(aes(x = sample_id, y = OTU, fill = RA)) +
  geom_tile() +
  scale_fill_gradientn(name="Relative abundance", 
                       colors = carto_pal(7, "BurgYl"), 
...
# Cluster plot
# Ensure the metadata (meta_hm_cpall_gf_op_order) is also ordered according to the same clustering order
meta_hm_cpall_gf_op_order <- meta_hm_cpall_gf_op_order[hc_cpall_gf_op_clust$order, ]
# Reorder the Sample IDs for Consistency
meta_hm_cpall_gf_op_order$sample_id <- factor(meta_hm_cpall_gf_op_order$sample_id,
                                              levels = hc_cpall_gf_op_clust$labels[hc_cpall_gf_op_clust$order])

# Reorder data for the Cluster Plot
meta_hm_cpall_gf_op_order$cluster <- factor(meta_hm_cpall_gf_op_order$cluster,
                                            levels =unique(meta_hm_cpall_gf_op_order$cluster[order(meta_hm_cpall_gf_op_order$cluster)]))
...
# Prevalence plot
# Using original phyloseq (not RA)
df_pres_op <- ps_cpall_gf %>%
  prune_samples(sample_data(.)$niche == "OP", .) %>%
  ps_to_df()
...
dim(df_pres_op)
# Convert counts to binary
df_pres_op[, colnames(df_pres_op)[700:20728]] <- lapply(df_pres_op[, colnames(df_pres_op)[700:20728]], function(x) ifelse(x > 2, 1, 0))
df_pres_op <- df_pres_op[700:20728]

prev <- colMeans(df_pres_op)
prev_op <- data.frame(Taxa = colnames(df_pres_op), Prevalence = prev)
prev_op$Taxa <- format_OTU(prev_op$Taxa)
prev_op <- subset(prev_op, Taxa %in% hm_cpall_gf_op_data$OTU)
prev_op$Taxa <- factor(prev_op$Taxa, levels = levels(hm_cpall_gf_op_data$OTU))
...
dendro_cpall_gf_op$plot + hm_pop_cpall_gf_op + hm_cluster_cpall_gf_op + hm_cpall_gf_op + p_prev_op + plot_layout(design = layout_cp_gf, guides = "collect") & theme(legend.position = 'bottom')
...
# Filter clusters with at least 10 individuals in both groups
...
# Ensure the population variable is a factor
...
for (clust in unique(sample_data_cp_gf_np_filt$cluster)) {
  ...
  # Extract coefficients and p-values if available
  model_coefficients <- summary(model_multivariable)$coefficients
  
  # Create a data frame of results for the current cluster
  cluster_results_np <- data.frame(
    Cluster = clust,
    Variable = rownames(model_coefficients),
    Estimate = model_coefficients[, "Estimate"],
    Std_Error = model_coefficients[, "Std. Error"],
    p_value = model_coefficients[, "Pr(>|z|)"]  # Correctly assign p-value
  )
  # Append cluster results to the overall results
  logistic_results_cp_gf_np <- rbind(logistic_results_cp_gf_np, cluster_results_np)
}


# Filter for rows where the Covariate column matches the specified covariates
filtered_results_cp_gf_np <- logistic_results_cp_gf_np %>%
  filter(Variable %in% "populationGF") 
# Print the filtered results
print(filtered_results_cp_gf_np)
# Apply the BH adjustment locally to the filtered covariates
filtered_results_cp_gf_np$BH_adjusted_pvalue <- p.adjust(filtered_results_cp_gf_np$p_value, method = "BH")
# Save as .csv
write.csv(filtered_results_cp_gf_np, "../Output/Cluster_analysis/CP_GF_NP/Logistic_regression_results_cp_gf_np.csv", row.names = FALSE)
...
log_regr_results_cp_gf_np <- filtered_results_cp_gf_np %>%
  select(Cluster, Variable, Estimate, BH_adjusted_pvalue)

# Calculate Odds Ratios and add the 'Variable' column
log_regr_results_cp_gf_np <- log_regr_results_cp_gf_np %>%
...
heatmap_data_cp_gf_np <- log_regr_results_cp_gf_np %>%
...
heatmap_data_cp_gf_np <- heatmap_data_cp_gf_np %>%
  left_join(log_regr_results_cp_gf_np %>% select(Variable, Cluster, Significance), by = c("Variable", "Cluster"))


# Replace "Pnpulation" with "Goat farmer vs controls" in the Variable column
...
# Save the heatmap
ggsave("../Output/Cluster_analysis/CP_GF_NP/Heatmap_cp_gf_np.svg", heatmap_occupational_expo_np, width = 5, height= 3)
```
...
log_regr_results_cp_gf_np <- filtered_results_cp_gf_np %>%
  select(Cluster, Variable, Estimate, BH_adjusted_pvalue, Std_Error) %>%
  mutate(
    OR = exp(Estimate),
...
# Combine data and assign colors to clusters
log_regr_results_cp_gf_np <- log_regr_results_cp_gf_np %>%
  mutate(Cluster = as.factor(Cluster))  # Ensure Cluster is treated as a factor

# Create a color palette from RColorBrewer (e.g., Set1, which is bright and distinct)
cluster_colors <- c("#873e23", "#76b5c5","#bd8edd")

# Create a single forest plot with updated appearance
log_regr_results_cp_gf_np$Significance
combined_forest_plot_np <- ggplot(log_regr_results_cp_gf_np, aes(x = OR, y = Cluster, color = Cluster)) +
  geom_point(size = 4, position = position_dodge(width = 0.7)) +  # Slightly larger points for visibility
  geom_errorbarh(aes(xmin = Lower_CI, xmax = Upper_CI), 
                 height = 0.2, position = position_dodge(width = 0.7)) +  # Error bars
  scale_color_manual(values = cluster_colors) +  # Updated colors for clusters
  labs(
    title = "Forest Plot of Odds Ratios\n Occupational exposure (GF vs CP)",
    x = "Odds Ratio (log scale)",
    y = NULL  # No label for y-axis
  ) +
  geom_vline(xintercept = 1, linetype = "dashed", color = "grey80", size = 1) +  # Dashed line at OR=1
  geom_text(aes(label = paste(Significance, sep = ""), color = Cluster), 
            position = position_dodge(width = 0.7), 
            hjust = 0.5, vjust = -0.5, size = 6)+  # p-value annotation above each cluster, aligned with point
  scale_x_log10() +  # Apply log scale to x-axis
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, size = 12),
    axis.title.x = element_text(size = 12),
    axis.title.y = element_blank(),  # No label for y-axis
    axis.text.y = element_text(size = 10),
    panel.grid.major.y = element_line(color = "grey90"),
    plot.margin = margin(1, 1, 1, 2), # Adjust plot margins for better spacing
    legend.position = "none"  # Remove the legend
  ) 

# Display the plot
print(combined_forest_plot_np)

# Save the plot
ggsave("../Output/Cluster_analysis/CP_GF_NP/Combined_forestplot_cp_gf_np.svg", 
       combined_forest_plot_np, width = 4, height = 4)
...
for (clust in unique(sample_data_cp_gf_op_filt$cluster)) {
  ...
  # Append cluster results to the overall results
  logistic_results_cp_gf_op <- rbind(logistic_results_cp_gf_op, cluster_results)
}


# Filter for rows where the Covariate column matches the specified covariates
filtered_results_cp_gf_op <- logistic_results_cp_gf_op %>%
  filter(Variable %in% "populationGF") 

# Print the filtered results
print(filtered_results_cp_gf_op)

# Apply the BH adjustment locally to the filtered covariates
filtered_results_cp_gf_op$BH_adjusted_pvalue <- p.adjust(filtered_results_cp_gf_op$p_value, method = "BH")
# Save as .csv
write.csv(filtered_results_cp_gf_op, "../Output/Cluster_analysis/CP_GF_OP/Logistic_regression_results_cp_gf_op.csv", row.names = FALSE)
...
# Select relevant columns from the original results DataFrame
log_regr_results_cp_gf_op <- filtered_results_cp_gf_op %>%
  select(Cluster, Variable, Estimate, BH_adjusted_pvalue)

# Calculate Odds Ratios and add the 'Variable' column
log_regr_results_cp_gf_op <- log_regr_results_cp_gf_op %>%
  mutate(OR = exp(Estimate),
         Variable = "Population")  # Ensure Variable column is created
...
# Reshape the data using the `dcast` function from the reshape2 package
heatmap_data_cp_gf_op <- log_regr_results_cp_gf_op %>%
  select(Variable, Cluster, OR, Significance) %>%  # Include 'Variable' and 'OR' in selection
  dcast(Variable ~ Cluster, value.var = "OR")  # Use the OR for reshaping
...
heatmap_data_cp_gf_op <- heatmap_data_cp_gf_op %>%
  left_join(log_regr_results_cp_gf_op %>% select(Variable, Cluster, Significance), by = c("Variable", "Cluster"))
...
# Save the heatmap
ggsave("../Output/Cluster_analysis/CP_GF_OP/Occupational_livestock_expo_heatmap_OP.svg", heatmap_occupational_expo_op, width = 5, height=3.5)

```
### Forest plot of odds ratios
```{r}
# Calculate Odds Ratios and Confidence Intervals
filtered_results_cp_gf_op$Std_Error
log_regr_results_cp_gf_op <- filtered_results_cp_gf_op %>%
  select(Cluster, Variable, Estimate, BH_adjusted_pvalue, Std_Error) %>%
  mutate(
    OR = exp(Estimate),
...
# Create a color palette from RColorBrewer (e.g., Set1, which is bright and distinct)
cluster_colors <- c("#43647b", "#c78780", "#7ca29a")

# Create a single forest plot with updated appearance
log_regr_results_cp_gf_op$Significance
combined_forest_plot_op <- ggplot(log_regr_results_cp_gf_op, aes(x = OR, y = Cluster, color = Cluster)) +
  geom_point(size = 4, position = position_dodge(width = 0.7)) +  # Slightly larger points for visibility
  geom_errorbarh(aes(xmin = Lower_CI, xmax = Upper_CI), 
                 height = 0.2, position = position_dodge(width = 0.7)) +  # Error bars
  scale_color_manual(values = cluster_colors) +  # Updated colors for clusters
...
# Display the plot
print(combined_forest_plot_op)

# Save the plot
ggsave("../Output/Cluster_analysis/CP_GF_OP/Combined_forestplot_cp_gf_op.svg", 
       combined_forest_plot_op, width = 4.5, height = 4)
...
boxplot(sample_data_df_cp_np$EU_ANNUALAVG_PM10_2020)
boxplot(sample_data_df_cp_np$ecoli_RF_preds)
boxplot(sample_data_df_cp_np$staph_RF_preds)
boxplot(sample_data_df_cp_np$tetw_RF_preds)
boxplot(sample_data_df_cp_np$meca_RF_preds)
boxplot(sample_data_df_cp_np$KRD_nPoultryWghtDist.3000m.sum)
boxplot(sample_data_df_cp_np$KRD_nGoatsWghtDist.3000m.sum)
...
## Logistic regression analysis - non-standardised variables
```{r}
...
for (clust in unique(sample_data_df_cp_np$cluster)) {
  ...
  for (rf_var in c("EU_ANNUALAVG_PM10_2020", "ecoli_RF_preds", "staph_RF_preds", "tetw_RF_preds", "meca_RF_preds")) {
...
logistic_results_df_cp_np$Covariate <- gsub("[0-9]+$", "", rownames(logistic_results_df_cp_np)) # This removes the number at the end of the variable - but also removes the 2020 from the EU_ANNUALAVG_PM10_ variable - so remove this from the next part to make sure it's included in the final dataframe

# Filter results and retain only rows of interest, then arrange columns for readability
# Define the covariates to filter by
covariates_to_keep <- c("EU_ANNUALAVG_PM10_", "ecoli_RF_preds", "staph_RF_preds", "tetw_RF_preds", "meca_RF_preds",
...
filtered_covariates_df_np <- logistic_results_df_cp_np %>%
  filter(Covariate %in% covariates_to_keep)
# Apply the BH adjustment locally to the filtered covariates
filtered_covariates_df_np$BH_adjusted_pvalue <- p.adjust(filtered_covariates_df_np$`Pr(>|z|)`, method = "BH")
# Save as .csv
write.csv(filtered_covariates_df_np, "../Output/Cluster_analysis/CP_NP/Logistic_regression_results_cp_np.csv")
...
log_regr_results_cp_np <- filtered_covariates_df_np %>%
  select(Cluster, Variable, Estimate, BH_adjusted_pvalue, `Std. Error`)

# Calculate Odds Ratios and Confidence Intervals
log_regr_results_cp_np <- log_regr_results_cp_np %>%
  mutate(
    OR = exp(Estimate),
...
log_regr_results_cp_np$Variable <- factor(log_regr_results_cp_np$Variable,
  levels = c("KRD_nCowsWghtDist.3000m.sum","KRD_nPoultryWghtDist.3000m.sum","KRD_nGoatsWghtDist.3000m.sum","KRD_nPigsWghtDist.3000m.sum","meca_RF_preds","tetw_RF_preds","staph_RF_preds","ecoli_RF_preds", "EU_ANNUALAVG_PM10_2020"))
...
heatmap_data_cp_np <- log_regr_results_cp_np %>%
...
heatmap_long_cp_np$Variable <- factor(
  heatmap_long_cp_np$Variable,
  levels = c(
    "Endotoxin exposure",
    "E. coli exposure",
    "Staph spp. exposure",
...
# Flip plot 
heatmap_residential_expo_cp_np_flip <- ggplot(heatmap_long_cp_np, aes(x = Variable, y = Cluster, fill = OR)) +
  geom_tile(color = "white") +
  scale_fill_gradient2(
    low = carto_pal(7, "Geyser")[1], 
    mid = carto_pal(7, "Geyser")[4], 
...
heatmap_residential_expo_cp_np_flip
# Save the heatmap
ggsave("../Output/Cluster_analysis/CP_NP/Residential_livestock_expo_heatmap_flip.svg", heatmap_residential_expo_cp_np_flip, width = 6, height = 4)
...
# Recode and reorder variables for cleaner labels
log_regr_results_cp_np <- log_regr_results_cp_np %>%
  mutate(Variable = factor(Variable, levels = c(
    ...
    "meca_RF_preds", "tetw_RF_preds", "staph_RF_preds", "ecoli_RF_preds", "EU_ANNUALAVG_PM10_2020"
  ))) %>%
...
unique_clusters <- unique(log_regr_results_cp_np$Cluster)
forest_plots <- list()  # Initialize an empty list to store plots

for (i in seq_along(unique_clusters)) {
  cluster <- unique_clusters[i]
  cluster_data <- log_regr_results_cp_np %>% filter(Cluster == cluster)
  
  # Dynamically adjust the theme for y-axis labels
  y_axis_theme <- if (i %% 2 == 1) {  # First plot in each row
    element_text(size = 10)  # Show y-axis labels
  } else {
    element_blank()  # Hide y-axis labels
  }
  
  # Create forest plot for the current cluster
  forest_plot <- ggplot(cluster_data, aes(x = OR, y = Variable)) +
    geom_point(aes(color = OR > 1), size = 3) +  # Color by OR > 1
    geom_errorbarh(aes(xmin = Lower_CI, xmax = Upper_CI), height = 0.2) +
    scale_color_manual(values = c("TRUE" = "red3", "FALSE" = "cyan4")) +
    labs(
      title = paste("Cluster", cluster),  # Add title here
      x = "Odds Ratio",
      y = NULL
    ) +  # <- Ensure `labs` is included with a `+`
    theme_minimal() +
    theme(
      plot.title = element_text(hjust = 0.5),
      panel.border = element_rect(color = "black", fill = NA, size = 0.5),  # Add border
      axis.text.y = y_axis_theme,  # Dynamically set y-axis labels
      legend.position = "none"  # Remove all legends
    )
  
  # Add plot to the list
  forest_plots[[cluster]] <- forest_plot
}


# Arrange the plots side by side
combined_plot <- wrap_plots(forest_plots, ncol = 2)  # Adjust number of columns as needed
print(combined_plot)

# Save the combined plot
ggsave("../Output/Cluster_analysis/CP_NP/Residential_livestock_expo_forestplot3.svg", combined_plot, width = 8, height = 10)
```
## Logistic regression analysis - standardised variables
```{r}
# Convert sample data to a dataframe
sample_data_df_cp_np <- data.frame(ps_RA_cpall_gf_np@sam_data)
...
heatmap_long_cp_np_std <- heatmap_long_cp_np_std %>%
  left_join(log_regr_results_cp_np_std %>% select(Variable, Cluster, Significance), by = c("Variable", "Cluster"))
...
# Save the heatmap
ggsave("../Output/Cluster_analysis/CP_NP/Residential_livestock_expo_heatmap_flip_standardised.svg", heatmap_residential_expo_cp_np_flip_std, width = 6, height = 4)
```
# OP 
## Cluster count and visualisation 
```{r}
...
sample_data_df_cp_op <- sample_data_df_cp_op %>%
  distinct(VGO3_ID, .keep_all = TRUE)
#### Logistic regression for continuous variables ####
# We conduct logistic regression to determine whether the probability of belonging to a specific cluster increases with higher values of each continuous variable.
# For RF-modelled concentrations, we use a multivariable logistic regression model per exposure agent i.e. cluster1 ~ e.coli + age + gender + season + smoking
...
covariates_to_keep <- c("ecoli_RF_preds", "staph_RF_preds", "tetw_RF_preds", "meca_RF_preds",
                        ...
                        "KRD_nPoultryWghtDist.3000m.sum", "KRD_nPigsWghtDist.3000m.sum")
# Filter the dataset to include only the specified covariates
filtered_covariates_df_op <- logistic_results_df_cp_op %>%
  filter(Covariate %in% covariates_to_keep)
# Apply the BH adjustment locally to the filtered covariates
filtered_covariates_df_op$BH_adjusted_pvalue <- p.adjust(filtered_covariates_df_op$`Pr(>|z|)`, method = "BH")
# Save as .csv
write.csv(filtered_covariates_df_op, "../Output/Cluster_analysis/CP_OP/Logistic_regression_results_cp_op.csv")
...
# Present the Odds ratio instead of the estimate - this tells us how the odds of belonging to a specific cluster change with each unit increase in the predictor variable. 
# only include columns of interest
log_regr_results_cp_op <- filtered_covariates_df_op %>%
  select(Cluster, Variable, Estimate, BH_adjusted_pvalue, `Std. Error`)

# Calculate Odds Ratios and Confidence Intervals
log_regr_results_cp_op <- log_regr_results_cp_op %>%
  mutate(
    OR = exp(Estimate),
    Lower_CI = exp(Estimate - 1.96 * `Std. Error`),
    Upper_CI = exp(Estimate + 1.96 * `Std. Error`)
...
# Reorder the variables for the plot
log_regr_results_cp_op$Variable <- factor(log_regr_results_cp_op$Variable,
                                          levels = c("KRD_nCowsWghtDist.3000m.sum","KRD_nPoultryWghtDist.3000m.sum","KRD_nGoatsWghtDist.3000m.sum","KRD_nPigsWghtDist.3000m.sum","meca_RF_preds","tetw_RF_preds","staph_RF_preds","ecoli_RF_preds"))
...
# Create heatmap data as before
heatmap_data_cp_op <- log_regr_results_cp_op %>%
  select(Variable, Cluster, OR, Significance) %>%
  dcast(Variable ~ Cluster, value.var = "OR")  # Reshape data for heatmap
...
heatmap_long_cp_op <- heatmap_long_cp_op %>%
  left_join(log_regr_results_cp_op %>% select(Variable, Cluster, Significance), by = c("Variable", "Cluster"))
...
# Reorder Variable factor levels
heatmap_long_cp_op$Variable <- factor(
  heatmap_long_cp_op$Variable,
  levels = c(
    "RF-modelled E. coli exposure",
    "RF-modelled Staphylococcus spp. exposure",
    "RF-modelled tetW ARG exposure",
    "RF-modelled mecA ARG exposure",
    "Distance weighted number of pigs in 3000m",
    "Distance weighted number of goats in 3000m",
    "Distance weighted number of chickens in 3000m",
    "Distance weighted number of cattle in 3000m"
  )
)

# Plot
heatmap_residential_expo_cp_op_flip <- ggplot(heatmap_long_cp_op, aes(x = Variable, y = Cluster, fill = OR)) +
  geom_tile(color = "white") +
  scale_fill_gradient2(
    low = carto_pal(7, "Geyser")[1], 
    mid = carto_pal(7, "Geyser")[4], 
  ...
  scale_x_discrete(labels = c(
    "RF-modelled E. coli exposure" = "E. coli exposure",
    "RF-modelled Staphylococcus spp. exposure" = "Staph spp. exposure",
    "RF-modelled tetW ARG exposure" = "tetW  exposure",
    "RF-modelled mecA ARG exposure" = "mecA exposure",
    "Distance weighted number of pigs in 3000m" = "Pigs",
    "Distance weighted number of goats in 3000m" = "Goats",
    "Distance weighted number of chickens in 3000m" = "Chickens",
    "Distance weighted number of cattle in 3000m" = "Cattle"
  )) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, size =14),  # Use element_text for line breaks
...
heatmap_residential_expo_cp_op_flip


# Save the heatmap
ggsave("../Output/Cluster_analysis/CP_OP/Residential_livestock_expo_heatmap_flip.svg", heatmap_residential_expo_cp_op_flip, width = 7, height = 4)
```
...
### Logistic regression analysis
# Convert sample data to a dataframe
sample_data_df_cp_op <- data.frame(ps_RA_cpall_gf_op@sam_data)
...
# Since Logistic regression requires sufficient counts in each outcome category to generate reliable and stable parameter estimates, we will exclude some of the clusters with low counts - anything with below 10 counts (out of the 1000 CPs)
# Calculate cluster counts
cluster_counts <- table(sample_data_df_cp_op$cluster)
# Identify clusters with counts >= 10
clusters_to_keep <- names(cluster_counts[cluster_counts >= 10])
# Filter the dataset to retain only rows with clusters in clusters_to_keep
sample_data_df_cp_op <- sample_data_df_cp_op %>%
...
# Initialize an empty data frame to store logistic regression results
logistic_results_df_cp_op_std <- data.frame()
...
for (clust in unique(sample_data_df_cp_op$cluster)) {
  ...
  summary_stats$Variable <- rownames(summary_stats)
  # Append to the results data frame
  logistic_results_df_cp_op_std <- rbind(logistic_results_df_cp_op_std, summary_stats)
  ...
  for (rf_var in c("EU_ANNUALAVG_PM10_2020_std", "ecoli_RF_preds_std", "staph_RF_preds_std", "tetw_RF_preds_std", "meca_RF_preds_std")) {
    ...
    rf_summary_stats$Variable <- rf_var
    # Append to the results data frame
    logistic_results_df_cp_op_std <- rbind(logistic_results_df_cp_op_std, rf_summary_stats)
  }
}

# Rearrange columns to show Cluster and Variable first (optional)
logistic_results_df_cp_op_std <- logistic_results_df_cp_op_std %>%
  select(Cluster, Variable, everything())
# Print save the results
print(logistic_results_df_cp_op_std)
# Add a new column "Covariate" to logistic_results_df_cp_op by removing numbers from the end of each Variable name
logistic_results_df_cp_op_std$Covariate <- gsub("[0-9]+$", "", rownames(logistic_results_df_cp_op_std)) # This removes the number at the end of the variable - but also removes the 2020 from the EU_ANNUALAVG_PM10_ variable - so remove this from the next part to make sure it's included in the final dataframe
...
# Filter the dataset to include only the specified covariates
filtered_covariates_df_op_std <- logistic_results_df_cp_op_std %>%
  filter(Covariate %in% covariates_to_keep)
# Apply the BH adjustment locally to the filtered covariates
filtered_covariates_df_op_std$BH_adjusted_pvalue <- p.adjust(filtered_covariates_df_op_std$`Pr(>|z|)`, method = "BH")
# Save as .csv
write.csv(filtered_covariates_df_op_std, "../Output/Cluster_analysis/CP_op/Logistic_regression_results_cp_op_standardised.csv")

# # Test BH-adjustment on all covariates in the models
# # Apply the BH adjustment globally across all p-values in the full dataset
# logistic_results_df_cp_op$BH_adjusted_pvalue_global <- p.adjust(logistic_results_df_cp_op$`Pr(>|z|)`, method = "BH")
...
# Present the Odds ratio instead of the estimate - this tells us how the odds of belonging to a specific cluster change with each unit increase in the predictor variable. 
# only include columns of interest
log_regr_results_cp_op_std <- filtered_covariates_df_op_std %>%
  select(Cluster, Variable, Estimate, BH_adjusted_pvalue, `Std. Error`)

# Calculate Odds Ratios and Confidence Intervals
log_regr_results_cp_op_std <- log_regr_results_cp_op_std %>%
  mutate(
    OR = exp(Estimate),
    Lower_CI = exp(Estimate - 1.96 * `Std. Error`),
    Upper_CI = exp(Estimate + 1.96 * `Std. Error`)
...
# Reorder the variables for the plot
log_regr_results_cp_op_std$Variable <- factor(log_regr_results_cp_op_std$Variable,
  levels = c("KRD_nCowsWghtDist.3000m.sum_std","KRD_nPoultryWghtDist.3000m.sum_std","KRD_nGoatsWghtDist.3000m.sum_std","KRD_nPigsWghtDist.3000m.sum_std","meca_RF_preds_std","tetw_RF_preds_std","staph_RF_preds_std","ecoli_RF_preds_std", "EU_ANNUALAVG_PM10_2020_std"))
...
# Create heatmap data as before
heatmap_data_cp_op_std <- log_regr_results_cp_op_std %>%
  select(Variable, Cluster, OR, Significance) %>%
  dcast(Variable ~ Cluster, value.var = "OR")  # Reshape data for heatmap

# Convert to long format for ggplot
heatmap_long_cp_op_std <- melt(heatmap_data_cp_op_std, id.vars = "Variable", variable.name = "Cluster", value.name = "OR")


# Merge significance markers back into the long format dataframe
heatmap_long_cp_op_std <- heatmap_long_cp_op_std %>%
  left_join(log_regr_results_cp_op_std %>% select(Variable, Cluster, Significance), by = c("Variable", "Cluster"))

# Reorder Variable factor levels
heatmap_long_cp_op_std$Variable <- factor(
  heatmap_long_cp_op_std$Variable,
  levels = c(
    "Endotoxin exposure",
    "E. coli exposure",
    "Staph spp. exposure",
...
# Flip plot 
heatmap_residential_expo_cp_op_flip_std <- ggplot(heatmap_long_cp_op_std, aes(x = Variable, y = Cluster, fill = OR)) +
  geom_tile(color = "white") +
  scale_fill_gradient2(
    low = carto_pal(7, "Geyser")[1], 
    mid = carto_pal(7, "Geyser")[4], 
...
heatmap_residential_expo_cp_op_flip_std
# Save the heatmap
ggsave("../Output/Cluster_analysis/CP_OP/Residential_livestock_expo_heatmap_op_flip_standardised.svg", heatmap_residential_expo_cp_op_flip_std, width = 7.8, height = 4)
```
...
hm1 <- heatmap_residential_expo_cp_np_flip_std + theme(legend.position = 'right', 
                                              plot.title = element_blank(),
                                              axis.title.x = element_blank(),
                                              axis.title.y = element_blank())
hm2 <- heatmap_residential_expo_cp_op_flip_std + theme(legend.position = 'none', 
                                              plot.title = element_blank(),
