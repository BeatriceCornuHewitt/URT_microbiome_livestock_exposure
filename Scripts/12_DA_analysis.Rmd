...
```
# Boxplots to check abundances - CP vs GF (NP)
```{r}
...
ggsave("../Output/DA_analysis/MaAsLin/CP_GF_NP/Genus_level/Significant_genera_violinplots.svg",
       width = 12, height = 8)
```
# Boxplots to check abundances - CP vs GF (OP)
```{r}
# Read in the MaAsLin2 results
maaslin_results_cp_gf_op <- read.csv("../Output/DA_analysis/MaAsLin/CP_GF_OP/Genus_level/maaslin_cp_gf_genus_op_ord.csv")

# Filter for significant results (e.g., FDR < 0.05)
significant_genera_cp_gf_op <- maaslin_results_cp_gf_op %>%
  filter(qval < 0.05) %>%
  arrange(desc(coef))
