...
cor_vars <- c("age", "gender", "sampling_season", "smoked_ever", 
              "KRD_nPigsWghtDist.3000m.sum", "KRD_nCowsWghtDist.3000m.sum", 
              "KRD_nPoultryWghtDist.3000m.sum", "KRD_nGoatsWghtDist.3000m.sum")
...
ecoli_expo_maaslin_res <- do_maaslin2_with_residuals(
  ps = ps_abs_cp_np_filt, 
  var = "ecoli_RF_preds",  # Including all livestock variables together
  other_var = c("age", "gender", "sampling_season", "smoked_ever"),  # Adjusting for these variables
  ref = c("gender,0", "sampling_season,Summer", "smoked_ever,0")  # Set reference categories
)
...
ps_abs_cp_np_filt@sam_data$log_cows <- log1p(ps_abs_cp_np_filt@sam_data$KRD_nCowsWghtDist.3000m.sum)
ps_abs_cp_np_filt@sam_data$log_goats <- log1p(ps_abs_cp_np_filt@sam_data$KRD_nGoatsWghtDist.3000m.sum)
ps_abs_cp_np_filt@sam_data$log_poultry <- log1p(ps_abs_cp_np_filt@sam_data$KRD_nPoultryWghtDist.3000m.sum )
ps_abs_cp_np_filt@sam_data$log_pigs <- log1p(ps_abs_cp_np_filt@sam_data$KRD_nPigsWghtDist.3000m.sum)
...
heatmap_cp_np_asvlevel <- ggplot(np_heatmap_data, aes(x = metadata, y = feature, fill = coef)) +
  ...
  geom_text(aes(label = label), color = "white", size = 5) +  # Add labels inside the cells
  theme_minimal() +
  theme(
...
heatmap_cp_op_asvlevel <- ggplot(op_heatmap_data, aes(x = metadata, y = feature, fill = coef)) +
  geom_tile(color = "black") +  # Create the heatmap tiles
  scale_fill_gradient2(
    low = carto_pal(7, "Geyser")[1], 
    mid = carto_pal(7, "Geyser")[4], 
    high = carto_pal(7, "Geyser")[7], 
...
# Log-transform the abundance values to reduce skew
ps_all_ASVs_cp_gf_np <- ps_all_ASVs_cp_gf_np %>%
  mutate(Abundance_log = log10(Abundance + 1)) # Add 1 to avoid log(0)

# nptional: Order ASVs alphabetically for consistent visualization
...
# Optional: Order ASVs alphabetically for consistent visualization
...
# Save the plot
ggsave("../Output/DA_analysis/All_ASVs_violinplots_op.svg",
       plot_all_ASVs, width = 16, height = 12)

```

## Maaslin
### CP vs GF (NP) ASV level 
```{r}
# ASV level results 
...
np_cp_gf_miseq_tse <- mia::makeTreeSummarizedExperimentFromPhyloseq(ps_np_cp_gf_abs_filt)
# Specify population levels
np_cp_gf_miseq_tse@colData$population <- factor(np_cp_gf_miseq_tse@colData$population, levels = c("CP", "GF"))

# Run ANCOM-BC2 analysis
ancom_np_all <- ancombc2(data = np_cp_gf_miseq_tse, 
...
ancom_np_primary <- ancom_np_all$res %>%
...
write.csv(ancom_np_primary, "../Output/DA_analysis/ANCOM-BC/ancom_np_primary.csv")


# Filter ASVs with q-value < 0.25 for the "population" variable
...
significant_ASVs_np <- ancom_np_primary %>%
  filter(q_populationGF < 0.25) # 10 ASVs that are significantly higher or lower

significant_ASVs_np_passedss <- ancom_np_primary %>%
  filter(q_populationGF < 0.25 & passed_ss_populationGF == TRUE) # 7 ASVs that was significantly different and passed the ss test
...
np_cp_gf_heatmap_data_ancom <- ancom_np_primary %>%
  mutate(
    label = case_when(
      q_populationGF < 0.05 ~ "***",  # Stars for qval < 0.05
      q_populationGF < 0.1 ~ "**",    # Stars for qval < 0.1
      q_populationGF < 0.25 ~ "*",    # Stars for qval < 0.25
...
heatmap_cp_gf_np_asvlevel_ancom_stars <- ggplot(np_cp_gf_heatmap_data_ancom, aes(x = "Population (GF vs CP)", y = taxon, fill = lfc_populationGF)) +
  geom_tile(color = "white") +  # Create the heatmap tiles
  scale_fill_gradient2(
    low = carto_pal(7, "Geyser")[1], 
    mid = carto_pal(7, "Geyser")[4], 
...
ggsave("../Output/DA_analysis/ANCOM-BC/Heatmap_ANCOMBC2_NP.svg", 
...
### OP
```{r}
op_cp_gf_miseq_tse <- mia::makeTreeSummarizedExperimentFromPhyloseq(ps_op_cp_gf_abs_filt)
saveRDS(op_cp_gf_miseq_tse, "../Output/DA_analysis/op_cp_gf_miseq_tse.rds")
op_cp_gf_miseq_tse@colData$population <- factor(op_cp_gf_miseq_tse@colData$population, levels = c("CP", "GF"))

# Run ANCOM-BC2 analysis
ancom_op_all <- ancombc2(data = op_cp_gf_miseq_tse, 
                         fix_formula = "population  + age + gender + sampling_season + smoked_ever",
                         rand_formula = NULL,
                         p_adj_method = "BH",
                         prv_cut = 0,
...
# Primary analysis results - also gives results from other factors included in the formula e.g. age, gender etc. 
ancom_op_primary <- ancom_op_all$res %>%
    mutate_if(is.numeric, function(x) round(x, 2))
...
write.csv(ancom_op_primary, "../Output/DA_analysis/ANCOM-BC/ancom_op_primary.csv")

# Filter ASVs with q-value < 0.25 for the "population" variable
significant_ASVs_op <- ancom_op_primary %>%
  filter(q_populationGF < 0.25) # 10 that are significantly higher or lower

significant_ASVs_op_passedss <- ancom_op_primary %>%
  filter(q_populationGF < 0.25 & passed_ss_populationGF == TRUE) # Only 2 ASVs that were significnat and passed the ss test
...
# Create the heatmap with stars inside the cells
heatmap_cp_gf_op_asvlevel_ancom_stars <- ggplot(op_cp_gf_heatmap_data_ancom, aes(x = "Population (GF vs CP)", y = taxon, fill = lfc_populationGF)) +
  geom_tile(color = "white") +  # Create the heatmap tiles
  scale_fill_gradient2(
    low = carto_pal(7, "Geyser")[1], 
    mid = carto_pal(7, "Geyser")[4], 
...
# Save the heatmap to a file
ggsave("../Output/DA_analysis/ANCOM-BC/Heatmap_ANCOMBC2_OP.svg", 
...
combined_heatmap_maaslin <- combined_heatmap_maaslin + 
  plot_annotation(title = "Heatmaps of MaAsLin Results for OP and NP Datasets") & 
...
```{r}
# Combine the two heatmaps side by side
combined_heatmap <- heatmap_cp_gf_np_asvlevel_ancom_stars + heatmap_cp_gf_op_asvlevel_ancom_stars +
                    plot_layout(ncol = 2)  # Arrange them in 2 columns

# Add a title for the combined plot
combined_heatmap <- combined_heatmap + 
  plot_annotation(title = "Heatmaps of ANCOM-BC Results for OP and NP Datasets") & 
  theme(plot.title = element_text(hjust = 0.5, size = 16, face = "bold"))
...
# Save the combined heatmap
ggsave("../Output/DA_analysis/ANCOM-BC/Combined_Heatmap_ANCOM_BC_OP_NP.svg", 
       combined_heatmap, width = 12, height = 8)

```

# Reporting for article 
```{r}
# MaAsLin2 tends to be more sensitive (detecting more associations), but this can come at the cost of a higher false-positive rate.
...
tax_table_ps <- tax_table(ps_new_metadata_RFpreds)
...
tax_table(ps_new_metadata_RFpreds)[7,]
