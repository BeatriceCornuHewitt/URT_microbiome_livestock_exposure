...
# Packages
```{r}
library(phyloseq); library(microbiomer); library(ggplot2); library(cowplot); library(patchwork); library(scales); library(tidyverse)
```
...
# SHANNON
# NP niche
lm_cattle2km_np <- lm(Shannon ~  KRD_nCowsWghtDist.3000m.sum + age + sampling_season + gender + smoked_ever, data = meta_np_cp)%>% 
...
# OP niche
lm_cattle2km_op <- lm(Shannon ~  KRD_nCowsWghtDist.3000m.sum + age + sampling_season + gender + smoked_ever, data = meta_op_cp) %>% 
  broom::tidy(., conf.int = T)
...
combined_data_shannon <- bind_rows(lm_cattle2km_np, lm_goats2km_np, lm_pigs2km_np, lm_poultry2km_np, lm_ecoliexpo_np, lm_staphexpo_np, lm_tetwexpo_np, lm_mecaexpo_np, lm_cattle2km_op, lm_goats2km_op, lm_pigs2km_op, lm_poultry2km_op, lm_ecoliexpo_op, lm_staphexpo_op, lm_tetwexpo_op, lm_mecaexpo_op)
...
ggsave("../Output/Alpha_diversity/livestock_expo_shannon.svg", livestock_expo_shannon, width = 8, height = 5)
...
# NP niche
lm_cattle2km_np_observed <- lm(Observed ~  KRD_nCowsWghtDist.3000m.sum + age + sampling_season + gender + smoked_ever, data = meta_np_cp)%>% 
  broom::tidy(., conf.int = T)
...
# OP niche
lm_cattle2km_op_observed <- lm(Observed ~  KRD_nCowsWghtDist.3000m.sum + age + sampling_season + gender + smoked_ever, data = meta_op_cp) %>% 
  broom::tidy(., conf.int = T)
...
# Combine all the tidy data
combined_data_observed <- bind_rows(lm_cattle2km_np_observed, lm_goats2km_np_observed, lm_pigs2km_np_observed, lm_poultry2km_np_observed, lm_ecoliexpo_np_observed, lm_staphexpo_np_observed, lm_tetwexpo_np_observed, lm_mecaexpo_np_observed, lm_cattle2km_op, lm_goats2km_op, lm_pigs2km_op, lm_poultry2km_op, lm_ecoliexpo_op_observed, lm_staphexpo_op_observed, lm_tetwexpo_op_observed, lm_mecaexpo_op_observed)
...
# Save 
ggsave("../Output/Alpha_diversity/livestock_expo_observed.svg", livestock_expo_observed,width = 8, height=5 )
```
### Model assumption checking
```{r}
# Check associations between Shannon and livestock exposure variables - check if linear
plot(meta_np_cp$KRD_nGoatsWghtDist.3000m.sum, meta_np_cp$Shannon,
     xlab = "Livestock Exposure (KRD_nGoatsWghtDist.3000m.sum)",
     ylab = "Shannon Diversity",
     main = "Shannon Diversity vs. Goat Exposure")
abline(lm(Shannon ~ KRD_nGoatsWghtDist.3000m.sum, data = meta_np_cp), col = "blue")


plot(meta_np_cp$KRD_nGoatFarm.3000m, meta_np_cp$Shannon,
     xlab = "Livestock Exposure (KRD_nGoatFarm.3000m)",
     ylab = "Shannon Diversity",
     main = "Shannon Diversity vs. number of goat farms in 2km")
abline(lm(Shannon ~ KRD_nGoatFarm.3000m, data = meta_np_cp), col = "blue")
...
plot(meta_np_cp$KRD_nAnyFarm_3000m, meta_np_cp$Shannon,
     xlab = "Livestock Exposure (KRD_nAnyFarm_3000m)",
     ylab = "Shannon Diversity",
     main = "Shannon Diversity vs. farm exposure")
abline(lm(Shannon ~ KRD_nAnyFarm_3000m, data = meta_np_cp), col = "blue")
