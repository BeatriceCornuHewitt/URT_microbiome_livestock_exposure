...
field_q <- field_q %>% 
  dplyr::rename(ID=fuhbid) %>% 
  dplyr::select(-X)
field_q$ID <- as.character(field_q$ID)
length(unique(field_q$ID)) # 969 in the home visit questionnaire
...
length(unique(field_f$ID)) # 969 visited at home 

length(unique(field_f$ID[field_f$fuhbfspirodone == "1"])) # 922 with spirometry done
length(unique(field_f$ID[field_f$fuhbfserumdone == "1"])) # 930 with serum done
...
length(unique(OQ$ID)) # 1071 completed online questionnaire
...
CP_metadata_all_filt %>%
  filter(fuhbforodone == "0" & fuhbfnasodone == "0") %>%
  summarise(unique_id_count = n_distinct(ID)) # 6 without OP or NP done
...
huisvrgl <-   dplyr::rename(huisvrgl, ID = fughbid)
...
webvrgl_newID <- webvrgl_newID %>%
    dplyr::select(-ID) %>%    # Remove the participantID column
    dplyr::rename(ID = name)             # Rename the name column to ID
...
```
# Rename metadata columns for CP, GP and GF 
```{r}
...
missing_variables_CP <- variable_names_CP[!(variable_names_CP %in% df_column_names_CP)]
...
RVO2021_livestock_exp <- read.csv("../Input/Livestock_computations/RVO2021_computations_max10km.csv")
...
VGO3_metadata_all_RVO <- left_join(VGO3_metadata_all, RVO2021_livestock_exp, by = "VGO3_ID")

# KRD data
KRD2022 <- read.csv("../Input/Livestock_computations/KRD2022_livestock_characteristics.csv")

KRD2022 <- KRD2022 %>%
  dplyr::rename(VGO3_ID = respnr)

KRD2022$VGO3_ID <- as.character(KRD2022$VGO3_ID)

VGO3_metadata_all_RVO_KRD <- left_join(VGO3_metadata_all_RVO, KRD2022, by = "VGO3_ID")


# RF-modelled microbial agents 
RF_exp <- read.csv("../Output/RF_models/All_RF_model_predictions_whole_cohort.csv")
...
RF_exp$VGO3_ID <- as.character(RF_exp$VGO3_ID)

VGO3_metadata_all_RVO_KRD_RF <- left_join(VGO3_metadata_all_RVO_KRD, RF_exp, by = "VGO3_ID")


# Dispersion-modelled endotoxin  
endotoxin <- read.csv("O:/DGK/IRAS/EEPI/Projects/VGO/VGO3_2tm3_4__gezondheid/Data/Livestock_exposure/VGO14k_endodisp_2012_2020.csv")
...
endotoxin$VGO3_ID <- as.character(endotoxin$VGO3_ID)

VGO3_metadata_all_RVO_KRD_RF_endo <- left_join(VGO3_metadata_all_RVO_KRD_RF, endotoxin, by = "VGO3_ID")
```
## Standardising all livestock predictor variables of interest
```{r}
# z-score standardisation to all livestock variables of interest in analyses
VGO3_metadata_all_RVO_KRD_RF_endo$EU_ANNUALAVG_PM10_2020
VGO3_metadata_all_RVO_KRD_RF_endo$ecoli_RF_preds
VGO3_metadata_all_RVO_KRD_RF_endo$staph_RF_preds
VGO3_metadata_all_RVO_KRD_RF_endo$tetw_RF_preds
VGO3_metadata_all_RVO_KRD_RF_endo$meca_RF_preds
VGO3_metadata_all_RVO_KRD_RF_endo$KRD_nCowsWghtDist.3000m.sum
VGO3_metadata_all_RVO_KRD_RF_endo$KRD_nGoatsWghtDist.3000m.sum
VGO3_metadata_all_RVO_KRD_RF_endo$KRD_nPoultryWghtDist.3000m.sum
VGO3_metadata_all_RVO_KRD_RF_endo$KRD_nPigsWghtDist.3000m.sum


# List of variables to standardize
vars_to_standardise <- c("EU_ANNUALAVG_PM10_2020", "ecoli_RF_preds", "staph_RF_preds", 
                         "tetw_RF_preds", "meca_RF_preds", "KRD_nCowsWghtDist.3000m.sum", 
                         "KRD_nGoatsWghtDist.3000m.sum", "KRD_nPoultryWghtDist.3000m.sum", 
                         "KRD_nPigsWghtDist.3000m.sum")

# Loop through each variable and standardize
for (var in vars_to_standardise) {
  new_var <- paste0(var, "_std")  # Create new variable name with '_std' suffix
  VGO3_metadata_all_RVO_KRD_RF_endo[[new_var]] <- scale(VGO3_metadata_all_RVO_KRD_RF_endo[[var]])
}
```
...
VGO3_metadata_all_RVO_KRD_RF_endo$visit_date <- as.Date(VGO3_metadata_all_RVO_KRD_RF_endo$visit_date)
month <- format(VGO3_metadata_all_RVO_KRD_RF_endo$visit_date, "%m")
...
VGO3_metadata_all_RVO_KRD_RF_endo$sampling_season <- sampling_season

# For pneumonia patients, all values of fuhbpneumonialy (pneumonia in the last year are NA), convert this to Yes
VGO3_metadata_all_RVO_KRD_RF_endo <- VGO3_metadata_all_RVO_KRD_RF_endo %>%
...
# Verify the changes
head(VGO3_metadata_all_RVO_KRD_RF_endo %>% select(population, COPD_emphysema_diag, COPD_status) %>% filter(population == "GP"))
...
# Verify the changes
head(VGO3_metadata_all_RVO_KRD_RF_endo %>% select(population, lung_med, lung_inhal_med) %>% filter(population == "GP"))
VGO3_metadata_all_RVO_KRD_RF_endo$lung_med[VGO3_metadata_all_RVO_KRD_RF_endo$population=="GP"]
...
VGO3_metadata_all_RVO_KRD_RF_endo$GoatFarmwithin2000m[VGO3_metadata_all_RVO_KRD_RF_endo$population=="CP"]
VGO3_metadata_all_RVO_KRD_RF_endo$GoatFarmwithin2000m[VGO3_metadata_all_RVO_KRD_RF_endo$population=="GP"]
VGO3_metadata_all_RVO_KRD_RF_endo$RVO2021_MindistGoatFarm50plus_NEG[VGO3_metadata_all_RVO_KRD_RF_endo$population=="CP"]
VGO3_metadata_all_RVO_KRD_RF_endo$RVO2021_MindistGoatFarm50plus_NEG[VGO3_metadata_all_RVO_KRD_RF_endo$population=="GP"]
...
VGO3_metadata_all_RVO_KRD_RF_endo$ChickenFarmExposureCategory[VGO3_metadata_all_RVO_KRD_RF_endo$population=="GF"]

# Create a new column ChickenFarm1000m - is the participant living < 1000m from a chicken farm
VGO3_metadata_all_RVO_KRD_RF_endo$ChickenFarm1000m <- ifelse(VGO3_metadata_all_RVO_KRD_RF_endo$RVO2021_nChickenFarm_1000m > 0, 1, 0)
# Create a new column ChickenFarm2000m - is the participant living < 2000m from a chicken farm
VGO3_metadata_all_RVO_KRD_RF_endo$ChickenFarm2000m <- ifelse(VGO3_metadata_all_RVO_KRD_RF_endo$RVO2021_nChickenFarm_2000m > 0, 1, 0)
...
VGO3_metadata_all_RVO_KRD_RF_endo$imm_def <- ifelse(VGO3_metadata_all_RVO_KRD_RF_endo$population %in% c("CP", "GF"), 
ifelse(rowSums(VGO3_metadata_all_RVO_KRD_RF_endo[, c("imm_def_med_current", "imm_def_illness_current", "imm_def_med", "imm_def_illness")], na.rm = TRUE) > 0, 1,
ifelse(rowSums(VGO3_metadata_all_RVO_KRD_RF_endo[, c("imm_def_med_current", "imm_def_illness_current", "imm_def_med", "imm_def_illness")], na.rm = TRUE) == 0, 0, NA)),
VGO3_metadata_all_RVO_KRD_RF_endo$imm_def)


sum(VGO3_metadata_all_RVO_KRD_RF_endo$population=="GP") # 214 (n.b. this is because it is separate for NP and OP)
sum(VGO3_metadata_all_RVO_KRD_RF_endo$population=="CP") # 969
sum(VGO3_metadata_all_RVO_KRD_RF_endo$population=="GF") # 100


# Remove duplicates for the GP population based on VGO3_ID
VGO3_metadata_all_RVO_KRD_RF_endo_nodups <- VGO3_metadata_all_RVO_KRD_RF_endo %>%
  group_by(population, VGO3_ID) %>%
  filter(row_number() == 1) %>%
  ungroup()

sum(VGO3_metadata_all_RVO_KRD_RF_endo_nodups$population=="GP") # 108 (duplicates removed)
sum(VGO3_metadata_all_RVO_KRD_RF_endo_nodups$population=="CP") # 969
sum(VGO3_metadata_all_RVO_KRD_RF_endo_nodups$population=="GF") # 100

dim(VGO3_metadata_all_RVO_KRD_RF_endo_nodups) # 1177 observations of 692 variables
dim(VGO3_metadata_all_RVO_KRD_RF_endo) # 1284 obs of 692 variables
```
# Sample metadata
```{r}
VGO3_metadata_all_livestock_forjoin <- VGO3_metadata_all_RVO_KRD_RF_endo
...
# Combine together
combined_df <- bind_rows(duplicated_df, VGO3_metadata_GP_df) # 2354 observations
...
VGO3_lab_missingrows$sample_id
# Combine the dataframes
VGO3_metadata_lab <- bind_rows(VGO3_metadata_all_lab, VGO3_lab_missingrows)
```

# Save all participant and sample metadata
```{r}
# This dataframe VGO3_metadata_all_RVO_KRD_RF_endo contains all metadata from participants and all livestock metadata (does not have GPs duplicated (i.e. metadata only for unique VGO3_ID))
write.csv(VGO3_metadata_all_RVO_KRD_RF_endo_nodups, "../Output/Metadata/VGO3_participant_metadata.csv")
...
write.csv(VGO3_metadata_lab, "../Output/Metadata/VGO3_participant_lab_metadata.csv")
```
# Descriptives of data
# Population descriptives (all cohort)
```{r, echo=TRUE, message=FALSE, warning=FALSE}
# These are descriptives for all participants enrolled in the VGO3 study who had a home visit (not necessarily had samples taken from them though)

VGO3_metadata_all_RVO_KRD_RF_endo_nodups %>%
  filter(population == "CP") %>%
  summarise(unique_id_count = n_distinct(VGO3_ID)) # 969
# Season
VGO3_metadata_all_RVO_KRD_RF_endo_nodups %>%
  filter(population == "CP") %>%
  group_by(sampling_season) %>%
  summarise(unique_id_count = n_distinct(VGO3_ID))
# 1 Autumn                      204
# 2 Spring                      339
# 3 Summer                      131
# 4 Winter                      295
...
VGO3_metadata_all_RVO_KRD_RF_endo_nodups %>%
  filter(population == "CP") %>%
  group_by(smoked_ever) %>%
  summarise(unique_id_count = n_distinct(VGO3_ID))
#   smoked_ever unique_id_count
#   <chr>                 <int>
# 1 0                       431
# 2 1                       483
# 3 2                        44
...
VGO3_metadata_all_RVO_KRD_RF_endo_nodups %>%
  filter(population == "CP") %>%
  group_by(asthma_ever) %>%
  summarise(unique_id_count = n_distinct(VGO3_ID))
#   asthma_ever unique_id_count
#   <chr>                 <int>
# 1 0                       892
# 2 1                        67
# 3 NA                       10
...
VGO3_metadata_all_RVO_KRD_RF_endo_nodups %>%
  filter(population == "CP") %>%
  group_by(COPD_emphysema_diag) %>%
  summarise(unique_id_count = n_distinct(VGO3_ID))

# abroad 
VGO3_metadata_all_RVO_KRD_RF_endo_nodups %>%
  filter(population == "CP") %>%
  group_by(abroad_lastmth) %>%
  summarise(unique_id_count = n_distinct(VGO3_ID))

# pneumococcal vaccine 
VGO3_metadata_all_RVO_KRD_RF_endo_nodups %>%
  filter(population == "CP") %>%
  group_by(pneumococcal_vacc) %>%
  summarise(unique_id_count = n_distinct(VGO3_ID))
...
str(VGO3_metadata_all_RVO_KRD_RF_endo_nodups$age)
VGO3_metadata_all_RVO_KRD_RF_endo_nodups$age <- as.numeric(VGO3_metadata_all_RVO_KRD_RF_endo_nodups$age)

VGO3_metadata_all_RVO_KRD_RF_endo_nodups %>%
  filter(population == "CP") %>%
  summarise(mean_age = mean(age, na.rm = TRUE))

# Number within 2000m of a goat farm
VGO3_metadata_all_RVO_KRD_RF_endo_nodups %>%
    filter(population == "CP") %>%
  filter(GoatFarmExposureCategory %in% c("<1000m", "1000-2000m")) %>%
  summarise(unique_id_count = n_distinct(VGO3_ID))

# Number within 1000m of a goat farm
VGO3_metadata_all_RVO_KRD_RF_endo_nodups %>%
  filter(population == "CP") %>%
  filter(GoatFarmExposureCategory %in% c("<1000m")) %>%
  summarise(unique_id_count = n_distinct(VGO3_ID))

# Number within 500m of a goat farm
VGO3_metadata_all_RVO_KRD_RF_endo_nodups %>%
  filter(population == "CP") %>%
  filter(GoatFarmwithin500m == 1) %>%
  summarise(unique_id_count = n_distinct(VGO3_ID))

# Any animal contact
VGO3_metadata_all_RVO_KRD_RF_endo_nodups %>%
  filter(population == "CP") %>%
  group_by(animal_contact_lastmth) %>%
  summarise(unique_id_count = n_distinct(VGO3_ID))


# All animals listed in questionnaire
VGO3_metadata_all_RVO_KRD_RF_endo_nodups %>%
  filter(population == "CP") %>%
  filter(
    dog_contact_lastmth == 1 |
    cat_contact_lastmth == 1 |
    rodent_contact_lastmth == 1 |
    bird_contact_lastmth == 1 |
    chicken_contact_lastmth == 1 |
    cow_contact_lastmth == 1 |
    pig_contact_lastmth == 1 |
    sheep_contact_lastmth == 1 |
    goat_contact_lastmth == 1 |
    horse_contact_lastmth == 1 |
    otheranim_contact_lastmth == !NA |
    animal_contact_lastmth == 1
  ) %>%
  summarise(unique_id_count = n_distinct(VGO3_ID)) 

# Companion animal contact - defined as dogs, cats, horses, 
VGO3_metadata_all_RVO_KRD_RF_endo_nodups %>%
  filter(population == "CP") %>%
  filter(
    dog_contact_lastmth == 1 |
...
# Livestock animal contact - defined as chickens, cows, pigs, sheep, goats, horses
VGO3_metadata_all_RVO_KRD_RF_endo_nodups %>%
  filter(population == "CP") %>%
  filter(
    chicken_contact_lastmth == 1 |
...
VGO3_metadata_all_RVO_KRD_RF_endo_nodups %>%
  ...
  summarise(unique_id_count = n_distinct(VGO3_ID))
VGO3_metadata_all_RVO_KRD_RF_endo_nodups_nohealthscore <- VGO3_metadata_all_RVO_KRD_RF_endo_nodups

VGO3_metadata_all_RVO_KRD_RF_endo_nodups_nohealthscore %>%
  filter(population == "CP") %>%
  summarise(
    total_rows = n(),
    missing_health_status = sum(is.na(health_status))
  )
str(VGO3_metadata_all_RVO_KRD_RF_endo_nodups_nohealthscore$health_status)
VGO3_metadata_all_RVO_KRD_RF_endo_nodups$health_status <- as.numeric(VGO3_metadata_all_livestock_nodups_nohealthscore$health_status)


VGO3_metadata_all_livestock_nodups_nohealthscore %>%
  filter(population == "CP") %>%
  summarise(
    mean_health_score = mean(health_status, na.rm = TRUE),
    max_health_score = max(health_status, na.rm = TRUE),
    min_health_score = min(health_status, na.rm = TRUE)
  )


# Pnuemonia in last year 
VGO3_metadata_all_livestock_nodups %>%
  filter(population == "CP") %>%
  group_by(pneumonia_lastyr) %>%
  summarise(unique_id_count = n_distinct(VGO3_ID))%>%
  mutate(percentage_of_total = (unique_id_count / sum(unique_id_count)) * 100)

# Pnuemonia in last 3 years
VGO3_metadata_all_livestock_nodups %>%
  filter(population == "CP") %>%
  group_by(pneumonia_last3yr) %>%
  summarise(unique_id_count = n_distinct(VGO3_ID))%>%
  mutate(percentage_of_total = (unique_id_count / sum(unique_id_count)) * 100)
...
VGO3_metadata_all_livestock_nodups %>%
  filter(population == "CP") %>%
  group_by(antacids_lastyr) %>%
  summarise(unique_id_count = n_distinct(VGO3_ID))%>%
  mutate(percentage_of_total = (unique_id_count / sum(unique_id_count)) * 100)

# Lung medication last year
VGO3_metadata_all_livestock_nodups %>%
  filter(population == "CP") %>%
  group_by(lung_med) %>%
  summarise(unique_id_count = n_distinct(VGO3_ID))%>%
...
# beta blocker medication last year
VGO3_metadata_all_livestock_nodups %>%
  filter(population == "CP") %>%
...
VGO3_metadata_all_livestock_nodups %>%
  filter(population == "CP") %>%
  group_by(acute_resp_infec_lastyr) %>%
  summarise(unique_id_count = n_distinct(VGO3_ID))%>%
  mutate(percentage_of_total = (unique_id_count / sum(unique_id_count)) * 100)
```
# Check some figures
