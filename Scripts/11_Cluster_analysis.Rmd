...
# Prevalence plot
# # Using original phyloseq (not RA)
# df_pres_op <- ps_cpall_gf %>% ps_to_df()
# # Check dimensions
# colnames(df_pres_op)
# dim(df_pres_op)
# # Convert counts to binary
# df_pres_op[, colnames(df_pres_op)[700:1989]] <- lapply(df_pres_op[, colnames(df_pres_op)[700:1989]], function(x) ifelse(x > 2, 1, 0))
# df_pres_op <- df_pres_op[700:1989]
# 
...
log_regr_results_cp_np <- log_regr_results_cp_np %>%
  mutate(Variable = factor(Variable, levels = c(
    ...
    "meca_RF_preds", "tetw_RF_preds", "staph_RF_preds", "ecoli_RF_preds"
  ))) %>%
...
ggsave("../Output/Cluster_analysis/CP_NP/Residential_livestock_expo_forestplot.svg", combined_plot, width = 8, height = 10)
...
log_regr_results_cp_gf_np <- log_regr_results_cp_gf_np %>%
  mutate(OR = exp(Estimate),
         Variable = "Pnpulation")  # Ensure Variable column is created
...
combined_forest_plot <- ggplot(log_regr_results_cp_gf_np, aes(x = OR, y = Cluster, color = Cluster)) +
...
# Save the plot
ggsave("../Output/Cluster_analysis/CP_GF_NP/Combined_forestplot_np.svg", 
       combined_forest_plot, width = 4, height = 4)
...
combined_forest_plot <- ggplot(log_regr_results_cp_gf_op, aes(x = OR, y = Cluster, color = Cluster)) +
  geom_point(size = 4, position = position_dodge(width = 0.7)) +  # Slightly larger points for visibility
  geom_errorbarh(aes(xmin = Lower_CI, xmax = Upper_CI), 
                 height = 0.2, position = position_dodge(width = 0.7)) +  # Error bars
  scale_color_manual(values = cluster_colors) +  # Updated colors for clusters
...
# Save the plot
ggsave("../Output/Cluster_analysis/CP_GF_OP/Combined_forestplot.svg", 
       combined_forest_plot, width = 4.5, height = 4)
